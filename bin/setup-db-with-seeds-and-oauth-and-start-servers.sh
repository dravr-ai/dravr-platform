...
# PIERRE_AGENTS_DIR set in .envrc or a sibling working-copy at
...
if [ ! -d "$AGENTS_DIR" ]; then
    ...
    echo "      next to this repo, or set PIERRE_AGENTS_DIR explicitly."
    exit 1
fi
"$PIERRE_CLI" seed agents --agents-dir "$AGENTS_DIR" 2>&1 | tail -3
