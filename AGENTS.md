...
- **Branch-lane coverage contract (decided 2026-08-31, carnet#154):** `ci-postgres.yml` runs the full server test suite (~600 files, ~21-24 min) on `main`, `schedule`, `workflow_dispatch`, and `feature/*` refs only. A push on `fix/*`/`debug/*`/`claude/*`/`copilot/*` runs the 8-file `*_postgresql_test.rs` smoke — a green `CI: Backend (PostgreSQL)` there means the smoke ran, NOT the suite. Per-push coverage on those refs = that smoke + `changed-server-tests` (SQLite, exactly this push's server test files) + `leaf-crate-tests` (every non-server crate). For a full verdict on any ref, dispatch ci-postgres. Coverage (~1h45m) is a SQLite run that fires weekly (Sunday 03:17 UTC), on `workflow_dispatch`, and on PRs — never on push, so it holds no slot during a main burst — and it is not a PG gate.
