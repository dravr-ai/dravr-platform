...
- Locally, only the tiers whose files changed run: Tier 0 `cargo fmt --all -- --check`, Tier 1 `scripts/ci/architectural-validation.sh`, Tier 1b `scripts/ci/check-contremaitre-sync.sh` (compile-free locale + notify-event drift check vs the pinned dravr-contremaitre catalogues), Tier 5 frontend, Tier 6 SDK, Tier 7 mobile.
...
The platform is coupled to **dravr-contremaitre** catalogues, and the tests that police that coupling run **full-suite-only** (cron SQLite shards + full-on-main ci-postgres + coverage). Because features merge via local squash with **no PR**, those tests first run *after* the squash is on main — so drift **false-greens the branch and reds main post-merge.** Mirror the change at authoring time:
...
`scripts/ci/check-contremaitre-sync.sh` (pre-push **Tier 1b**) catches the locale + notify drifts *before* you push — compile-free. It does **not** cover the tool list (names aren't string-literal at a greppable site), so the `McpTool` step above is still on you.
</important>
