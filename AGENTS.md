...
**Backend tests** — full suite is ~13 min across ~600 binaries. ALWAYS target a file:
...
Find a test's file: `rg "test_name" tests/ --files-with-matches`. NEVER `cargo test <name>` without `--test` (compiles all ~600 files); NEVER `cargo test --lib` (runs ~0 tests).
