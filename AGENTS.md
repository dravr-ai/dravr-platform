...
- **Binary size: keep `pierre_mcp_server` <= 95MB** (CI's `release-binary` job checks this; raised from 80 on 2026-09-03, when main had sat at exactly 80MB for four runs with no headroom, and from 90 on 2026-09-18, when main had crept to 88MB and the copilot_sdk provider's generated protocol types made 91 — the levers before the next raise are the duplicate `reqwest`/`tokio-tungstenite` versions in the lockfile and fat LTO for the shipped binary). Watch large deps; use feature flags to drop unused code; document any justified exception.
</important>
