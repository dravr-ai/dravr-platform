...
function RootLayoutNav() {
  ...
  // The two faces beside the system one (Boreal v2.2 D4): Schibsted Grotesk
  // 600 for the wordmark and the auth and onboarding headline, JetBrains Mono
  // for counts, times and code. Keys match the family names declared in
  // tailwind.config.js (`SchibstedGrotesk`, `JetBrainsMono`) so `font-display`
  // and `font-mono` resolve once loaded; every other string renders in the
  // platform face, which is what Dynamic Type and the native chrome use.
  const [fontsLoaded] = useFonts({
    SchibstedGrotesk: SchibstedGrotesk_600SemiBold,
  ...
  // and map it to its `(onboarding)` route — or route to Home when none remain.
  React.useEffect(() => {
    if (isLoading) return;
