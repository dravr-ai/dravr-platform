...
impl PerformanceAnalyzerV2 {
    ...
    pub fn calculate_fitness_score(&self, activities: &[Activity]) -> Result<FitnessScore> {
        ...
        #[allow(clippy::suboptimal_flops)] // Readability over micro-optimization
        let overall_score = (aerobic_fitness * self.config.fitness_scoring.aerobic_weight)
            + (strength_endurance * self.config.fitness_scoring.strength_weight)
    ...
    pub fn predict_performance(
        ...
        #[allow(clippy::cast_precision_loss)] // Safe: Small counts fit in f64 mantissa
        let future_x = data_points.len() as f64 + (days_to_target as f64 / 7.0); // Weekly progression
        #[allow(clippy::suboptimal_flops)] // Clearer mathematical formula
        let predicted_value = regression.slope * future_x + regression.intercept;
