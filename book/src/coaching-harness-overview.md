...
| 3 | Agent-authored memory tools | `agent_note_add`, `agent_followup_schedule`, `remember_fact`, `recall_user_memory` MCP tools the agent can call to write its own memory | `tools/implementations/memory.rs`, `tools-memory` feature flag |
