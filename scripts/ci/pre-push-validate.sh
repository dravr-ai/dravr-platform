            ...
            case "$file" in
                ...
                crates/pierre-server/tests/*.rs)
                    if [[ "$file" =~ ^crates/pierre-server/tests/[^/]+\.rs$ ]]; then
                        ...
                        # llm_local_integration_test requires Ollama on localhost:11434 (CI
                        # provisions it; local dev machines usually don't have it running).
                        ...
                        if [[ -f "$PROJECT_ROOT/$file" \
                              ...
                              && "$test_name" != "helpers" \
                              && "$test_name" != "fixtures" \
                              && "$test_name" != "messaging_commands_test" \
                              && "$test_name" != "llm_local_integration_test" ]]; then
                            add_tests "$test_name"
                        fi
                    fi
                    ;;
