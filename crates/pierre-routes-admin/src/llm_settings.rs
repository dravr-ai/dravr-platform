...
use pierre_services::chat_provider_factory::chat_provider_from_credentials;
...
impl LlmSettingsRoutes {
    ...
    {
        Router::new()
            .route("/api/user/llm-settings", get(get_llm_settings::<C>))
            .route("/api/user/llm-settings", put(save_llm_credentials::<C>))
            .route(
                "/api/user/llm-settings/validate",
                post(validate_llm_credentials::<C>),
            )
            .route(
                "/api/user/llm-settings/{provider}",
                delete(delete_llm_credentials::<C>),
            )
...
/// No user-id fallback.
async fn get_tenant_id<C: MiddlewareCtx>(
    auth: &AuthResult,
    resources: &Arc<C>,
...
async fn get_llm_settings<C: MiddlewareCtx>(
    ...
    let all_credentials = TenantLlmManager::list_tenant_credentials(tenant_id, llm_creds).await?;
...
async fn save_llm_credentials<C: MiddlewareCtx>(
    State(resources): State<Arc<C>>,
    auth: AuthenticatedUser,
...
async fn validate_llm_credentials<C: MiddlewareCtx>(
    State(resources): State<Arc<C>>,
    auth: AuthenticatedUser,
...
async fn delete_llm_credentials<C: MiddlewareCtx>(
    State(resources): State<Arc<C>>,
    auth: AuthenticatedUser,
    ...
    let deleted =
        TenantLlmManager::delete_credentials(Some(user_id), tenant_id, provider, llm_creds).await?;
