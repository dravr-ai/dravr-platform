...
impl ImpersonationRoutes {
        ...
        C: MiddlewareCtx,
    {
        Router::new()
            .route(
                ...
                post(Self::handle_start_impersonation::<C>),
            )
            .route(
                "/api/admin/impersonate/end",
                post(Self::handle_end_impersonation::<C>),
            )
            .route(
                "/api/admin/impersonate/sessions",
                get(Self::handle_list_sessions::<C>),
            )
            .route(
                "/api/admin/impersonate/sessions/{session_id}",
                get(Self::handle_get_session::<C>),
            )
            .with_state(resources)
    }

    /// Authenticate user and require super admin role
    async fn authenticate_super_admin<C: MiddlewareCtx>(
        headers: &HeaderMap,
        resources: &Arc<C>,
    ...
    async fn handle_start_impersonation<C: MiddlewareCtx>(
    ...
    async fn handle_end_impersonation<C: MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        headers: HeaderMap,
    ) -> Result<Response, AppError> {
    ...
    async fn handle_list_sessions<C: MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        headers: HeaderMap,
    ) -> Result<Response, AppError> {
        // Authenticate and verify super admin status
    ...
    async fn handle_get_session<C: MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        headers: HeaderMap,
