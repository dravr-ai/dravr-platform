// ABOUTME: Admin route for the tenant-wide agent_notes compliance audit log
// ABOUTME: Exposes every note an agent persona has authored about any user in the tenant
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! Admin agent note audit log route.
//!
//! Agent personas authoring notes about users creates a privacy and
...
//! having to enumerate (user, agent) pairs.
...
pub struct AuditQuery {
    /// Tenant to list agent notes for (admin tokens can span tenants).
    pub tenant_id: String,
    /// Maximum rows to return, clamped to `1..=500`. Defaults to 100.
    pub limit: Option<i64>,
}

/// Wire-format audit row for a single agent note.
///
/// The domain [`AgentNote`] keeps scope as an enum — this struct flattens
/// it to a stable `snake_case` string so the `TypeScript` client renders
/// without an extra mapping.
#[derive(Debug, Serialize)]
pub struct AgentNoteAuditRow {
    ...
    pub scope: String,
    /// Free-form note content recorded by the agent.
    pub content: String,
...
impl From<AgentNote> for AgentNoteAuditRow {
    fn from(n: AgentNote) -> Self {
        Self {
            id: n.id,
...
#[derive(Debug, Serialize)]
pub struct AgentNoteAuditResponse {
    /// Audit rows for this page.
    pub notes: Vec<AgentNoteAuditRow>,
...
///
/// Requires [`AdminPermission::ViewAuditLogs`] — agent notes contain
...
pub(crate) async fn handle_list_audit(
    ...
    let notes = context
        .repos
        .memory
        .list_agent_notes_for_tenant(tenant, limit)
        .await
        .map_err(|e| {
    ...
    let rows: Vec<AgentNoteAuditRow> = notes.into_iter().map(Into::into).collect();
    let total = rows.len();
    ...
    Ok((
        StatusCode::OK,
        Json(AgentNoteAuditResponse { notes: rows, total }),
    ))
}
...
async fn set_suppressed(
    ...
    let changed = context
        .repos
        .memory
        .set_agent_note_suppressed(&note_id, tenant, suppressed, &admin_token.service_name)
        .await
        .map_err(|e| {
