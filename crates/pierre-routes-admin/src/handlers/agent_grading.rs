// ABOUTME: Admin route exposing per-agent claim-verification content grades for store ranking review
// ABOUTME: Wraps services::agent_grading::compute_agent_grades behind ViewConfiguration
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! Admin agent content grading route.
//!
//! Wraps [`pierre_services::agent_grading::compute_agent_grades`] in
//! an axum handler. Requires [`AdminPermission::ViewConfiguration`]
//! since the underlying data (`claim_verdicts`) is the same source
//! the claim-verdicts triage tab uses.

use std::sync::Arc;

use axum::{
    extract::{Query, State},
    http::StatusCode,
    response::IntoResponse,
    Extension, Json,
};
use serde::Deserialize;
use tracing::{error, info};

use pierre_core::admin::models::{AdminPermission, ValidatedAdminToken};
use pierre_core::errors::{AppError, AppResult};
use pierre_core::models::TenantId;
use pierre_services::agent_grading::{
    compute_agent_grades, DEFAULT_VERDICT_LIMIT, MAX_VERDICTS_SCANNED,
};

use crate::context::AdminApiContext;

/// Query parameters for the agent grading summary endpoint.
#[derive(Debug, Deserialize)]
pub struct AgentGradingQuery {
    /// Tenant to compute grades for (admin tokens span tenants).
    pub tenant_id: String,
    /// Maximum verdicts to scan, clamped to `1..=MAX_VERDICTS_SCANNED`.
    pub limit: Option<i64>,
}

/// Handle `GET /admin/agent-grading/summary`.
pub(crate) async fn handle_get_summary(
    State(context): State<Arc<AdminApiContext>>,
    Extension(admin_token): Extension<ValidatedAdminToken>,
    Query(params): Query<AgentGradingQuery>,
) -> AppResult<impl IntoResponse> {
    admin_token.require_permission(&AdminPermission::ViewConfiguration)?;

    let tenant = TenantId::parse_str(&params.tenant_id)
        .map_err(|_| AppError::invalid_input(format!("Invalid tenant ID: {}", params.tenant_id)))?;
    let limit = params
        .limit
        .unwrap_or(DEFAULT_VERDICT_LIMIT)
        .clamp(1, MAX_VERDICTS_SCANNED);

    let summary = compute_agent_grades(&context.repos.agent_repos(), tenant, limit)
        .await
        .map_err(|e| {
            error!(error = %e, "failed to compute coach grading summary");
            AppError::internal(format!("Failed to compute coach grading summary: {e}"))
        })?;

    info!(
        service = %admin_token.service_name,
        tenant = %params.tenant_id,
        verdicts_scanned = summary.verdicts_scanned,
        agents = summary.grades.len(),
        "admin fetched coach grading summary"
    );

    Ok((StatusCode::OK, Json(summary)))
}
