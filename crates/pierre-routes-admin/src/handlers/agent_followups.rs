// ABOUTME: Admin routes for the tenant-wide agent followup triage queue
// ABOUTME: List pending followups and cancel stale promises from the admin UI
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! Admin agent followup triage routes.
//!
//! Agent personas promise future check-ins ("I'll ask about your Achilles
//! tomorrow") via the followup tool. The harness stores those promises in
//! `agent_followups` and injects the pending ones into the next system
//! prompt. This admin surface lets tenant admins review the pending queue
//! across all users and agents, and cancel stale promises that should
...
///
/// Flattens the domain [`AgentFollowup`] enum to a stable string for the
/// status column so the `TypeScript` client can render without mapping.
#[derive(Debug, Serialize)]
pub struct FollowupRow {
    /// Stable identifier for this followup row.
    pub id: String,
    /// Tenant that owns the followup.
    pub tenant_id: String,
    /// User the followup is addressed to.
    pub user_id: String,
    /// Agent that scheduled the followup.
    pub agent_id: String,
    /// Conversation the followup originated from, when known.
    pub conversation_id: Option<String>,
    /// Followup message body to be re-injected on delivery.
    pub content: String,
    /// RFC3339 timestamp the followup becomes eligible for delivery.
    pub due_at: Option<String>,
    /// Lifecycle status (`pending`, `delivered`, `cancelled`).
    pub status: String,
    /// RFC3339 timestamp the followup was created.
    pub created_at: String,
    /// RFC3339 timestamp the followup was last updated.
    pub updated_at: String,
    /// RFC3339 timestamp the followup was delivered, when applicable.
    pub delivered_at: Option<String>,
}

impl From<AgentFollowup> for FollowupRow {
    fn from(f: AgentFollowup) -> Self {
        Self {
            id: f.id,
...
pub struct FollowupListResponse {
    ...
    /// Number of rows returned in `followups`.
    pub total: usize,
}
...
/// will not inject it into the next agent system prompt. Requires
