// ABOUTME: MCP wire protocol schema definitions for Model Context Protocol communication
// ABOUTME: Leaf crate with only serde as a runtime dep — no pierre-server internals
//
...
//! # Pierre MCP Schema
//!
//! Type-safe definitions for MCP protocol messages, capabilities, and tool schemas.
//! Imported by `pierre-server` and any other crate that needs to emit or parse MCP
//! wire frames. Kept free of pierre-server internals so it can be a leaf in the
//! workspace dependency graph; the `InitializeResponse` builder takes the public
//! OAuth origin (`base_url`) as a parameter rather than reading it from a static
//! `ServerConfig`, so the caller resolves runtime config and passes a plain
//! `String` in.
...
// MCP wire-frame types are layered on top of the protocol-agnostic JSON-RPC
// foundation in `pierre-jsonrpc`. The aliases below give MCP-specific names
// to the shared structs so the rest of the codebase can keep speaking in
// MCP-flavored terminology while sharing a single canonical implementation
// with A2A and any future JSON-RPC transport.

/// MCP request wire frame (alias for the canonical JSON-RPC request)
pub type McpRequest = pierre_jsonrpc::JsonRpcRequest;
/// MCP response wire frame (alias for the canonical JSON-RPC response)
pub type McpResponse = pierre_jsonrpc::JsonRpcResponse;
/// MCP error object (alias for the canonical JSON-RPC error)
pub type McpError = pierre_jsonrpc::JsonRpcError;
...
impl InitializeResponse {
    /// Create a new initialize response. `base_url` is the public origin used to
    /// build the `OAuth2` capability endpoints (e.g. `<https://api.example.com>`).
    /// The caller resolves this from `pierre-server`'s runtime config — the schema
    ...
    pub fn new(
        ...
        let oauth2_capability = OAuth2Capability {
        ...
        Self {
            ...
            capabilities: ServerCapabilities {
                ...
                auth: Some(AuthCapability {
                    oauth2: Some(oauth2_capability.clone()),
                }),
                oauth2: Some(oauth2_capability),
