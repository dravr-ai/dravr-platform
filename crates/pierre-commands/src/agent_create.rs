// ABOUTME: Handler for /agent create — drafts an agent from the conversation, parks it, and creates it on confirm
...
//! `/agent create` — an agent drafted from the conversation, confirmed to exist.
//!
...
//! the agent too, because the claim path routes on the parked action's kind.
...
use crate::agent::{bind_agent, AgentBinding};
use crate::guardian_confirm::resolve_pending;
use crate::{CommandHandler, PlatformCommandContext};

/// `tool_name` a parked agent draft carries in the pending-action store.
///
/// Deliberately not a registered tool: the claim path checks for it before
/// anything is dispatched, so a confirmed draft becomes an agent here rather
/// than a tool call the executor would refuse.
pub const AGENT_PROPOSAL_ACTION: &str = "coach.create_proposal";
...
/// The draft as stored between `/agent create` and its confirmation.
...
impl From<AgentProposal> for ParkedProposal {
...
/// Handler for `/agent create` and `/agent create confirm <token>`.
///
...
/// Draft an agent from the conversation the command was typed in, park it,
...
async fn propose(ctx: &PlatformCommandContext) -> Result<CommandResponse, AppError> {
    ...
    let proposal = propose_agent(&provider, &ctx.ctx.agent_generation_prompt(), &excerpt).await?;
...
///
/// Filed under the caller's own tenant — the tenant `/agent create confirm`
/// and `/deny` claim with — even when the conversation lives under a shared
/// room's channel tenant.
async fn park(ctx: &PlatformCommandContext, proposal: AgentProposal) -> Result<String, AppError> {
...
/// Create the agent a claimed draft describes and bind it to the
/// conversation.
///
/// Reached from the claim path once the athlete's `confirm` won the
/// single-use claim. The per-user agent quota is the one `POST /api/agents`
/// enforces, read at this moment rather than at drafting time so an agent
/// deleted in between counts. The new agent is given its catalogue handle
/// right away — `@handle` and `/agent add @handle` reach it from its first
/// second — and attached exactly the way `/agent add` attaches one.
pub(crate) async fn create_from_claimed_proposal(
    ...
    let request = CreateAgentRequest {
        ...
        category: AgentCategory::parse(&parked.category),
        tags: parked.tags,
        sample_prompts: vec![],
        startup_query: None,
    ...
    let handle = repos
        .store_listings
        .assign_catalogue_handle(&agent_id, ctx.tenant_id)
        .await?;
    let agent = Agent {
    ...
    let key = match bind_agent(ctx, &agent).await? {
        AgentBinding::Personal | AgentBinding::Group(_) => KEY_AGENT_CREATE_DONE,
        AgentBinding::Refused => KEY_AGENT_CREATE_DONE_UNBOUND,
    };
