...
/// Maximum number of agents offered as buttons on the list card.
const MAX_AGENT_BUTTONS: usize = 8;

/// Handler for `/agent` (also `/agent list`, and the legacy `/agent` spellings) — the caller's own
/// agent list as an interactive card.
///
/// The list is what `find_installed_by_handle` resolves against: the agents
/// the caller created and the ones they installed from Discover. System
/// agents they never installed are deliberately absent — `/discover` is the
...
impl CommandHandler for AgentListHandler {
    async fn execute(&self, ctx: &PlatformCommandContext) -> Result<CommandResponse, AppError> {
        ...
        ctx.ctx
            .repos()
            .agents
            .apply_translations(&mut agents, locale)
            .await?;
        ...
        for item in &agents {
            ...
            // Agent markdown files use CommonMark emphasis (*x*, **x**, _x_,
            ...
            let line = item.agent.handle.as_deref().map_or_else(
                || {
                    reg.render(
                        KEY_AGENT_LIST_ITEM_NO_HANDLE,
                        locale,
                        &[&item.agent.title, &desc],
                    )
                },
                |handle| {
                    reg.render(
        ...
        let actions: Vec<CommandAction> = agents
            .iter()
            .take(MAX_AGENT_BUTTONS)
            .map(|item| CommandAction {
                label: item.agent.title.clone(),
                action_type: "postback".to_owned(),
                value: add_postback(&item.agent),
            })
            .collect();
...
/// The `/agent add` text that names `agent`: its handle when it owns one,
...
/// [`AgentHandle::MAX_LEN`] characters is 52 bytes, and the id form is 47 —
/// both under the 64-byte ceiling Telegram puts on a button's callback data.
fn add_postback(agent: &Agent) -> String {
    agent.handle.as_deref().map_or_else(
        || format!("/agent add {}", agent.id),
...
/// form the list card sends for an agent that owns no handle). Either way the
/// agent has to be on the caller's list: a catalogue agent they never
/// installed is refused by name, and no `/discover install` happens on their
/// behalf. Everything after the lookup is [`bind_agent`].
///
...
impl CommandHandler for AgentAddHandler {
    async fn execute(&self, ctx: &PlatformCommandContext) -> Result<CommandResponse, AppError> {
        ...
        let Some(agent) = resolve_listed_agent(ctx, typed).await? else {
        ...
        let binding = bind_agent(ctx, &agent).await?;
        let text = match &binding {
            AgentBinding::Personal => reg.render(KEY_AGENT_USER_UPDATED, locale, &[&agent.title]),
            AgentBinding::Group(group_name) => {
                reg.render(KEY_AGENT_GROUP_UPDATED, locale, &[&agent.title, group_name])
            }
            AgentBinding::Refused => reg.render(KEY_AGENT_ASSIGN_FORBIDDEN, locale, &[]),
        };
        Ok(CommandResponse::text(text))
    }
}

/// The caller's own agent list: the agents they created and the ones they
/// installed from Discover — the set `/agent` shows and `/agent add` resolves
/// against. System agents they never installed are not on it.
fn installed_filter() -> ListAgentsFilter {
    ListAgentsFilter {
...
///
/// A catalogue handle resolves through `find_installed_by_handle`. An agent id
/// — the form the list card sends for an agent that owns no handle — resolves
/// within that same list, so neither form reaches an agent the caller never
/// installed. A token the handle grammar refuses (`@Agent Tempo`) is the same
/// case as a well-formed handle nobody owns: neither names a listed agent.
async fn resolve_listed_agent(
...
/// Where an agent ended up after [`bind_agent`].
pub(crate) enum AgentBinding {
    /// A personal conversation: the selection pointer moved and the
    /// conversation rebound.
    Personal,
    /// A group conversation: the named group's agent changed and the
    /// conversation rebound.
    Group(String),
...
/// Make `agent` answer in the conversation the command was typed in.
///
...
/// [`bind_conversation_agent`]), so the agent answers from the next message
/// on. In a group conversation the agent becomes the group's — a settings
...
pub(crate) async fn bind_agent(
    ...
    if ctx.is_direct_message {
        ctx.ctx
            .repos()
            .tenants
            .set_selected_agent(ctx.tenant_id, ctx.user_id, Some(agent_id))
            .await?;
        bind_conversation_agent(ctx, agent_id).await?;
        record_slash_selection(ctx, agent_id).await;
        return Ok(AgentBinding::Personal);
    }
    ...
    if !AgentAssignHandler::permits(member.role) {
...
/// Point the conversation the command was typed in at `agent_id`.
///
/// The selection pointer alone reaches only conversations opened *after* it:
/// a web thread bound at creation kept its old agent for as long as it lived,
/// and a messaging thread waited for the next inbound turn to notice. Writing
/// the row here is what makes adding an agent mean it answers the next
...
async fn bind_conversation_agent(
    ctx: &PlatformCommandContext,
    agent_id: &str,
) -> Result<(), AppError> {
    ...
    chat.set_conversation_agent_id(conversation_id, Some(agent_id), ctx.conversation_tenant_id)
        .await?;
    Ok(())
}
...
///
/// Personal conversations only: in a group conversation the agent is the
/// group's, and `/group agent` is the command that changes it. Clears both
...
/// would bring the agent back with the athlete's next message.
pub struct AgentRemoveHandler;
...
impl CommandHandler for AgentRemoveHandler {
    async fn execute(&self, ctx: &PlatformCommandContext) -> Result<CommandResponse, AppError> {
        ...
        // The title is only decoration on the confirmation; an agent deleted
        ...
        let title = ctx
            ...
            .await?
            .map_or_else(|| "Agent".to_owned(), |agent| agent.title);

        chat.set_conversation_agent_id(conversation_id, None, ctx.conversation_tenant_id)
            .await?;
        ctx.ctx
            .repos()
            .tenants
            .set_selected_agent(ctx.tenant_id, ctx.user_id, None)
            .await?;
...
/// Handler for `/agent assign <agent_id> <group_id>` — bind an agent to a specific group
pub struct AgentAssignHandler;

impl AgentAssignHandler {
    /// The single authority on who may change a group's agent, shared by
    /// `execute`, `is_available` and the group branch of [`bind_agent`].
...
#[async_trait]
impl CommandHandler for AgentAssignHandler {
    async fn execute(&self, ctx: &PlatformCommandContext) -> Result<CommandResponse, AppError> {
        let agent_id = ctx
            .args
            .first()
        ...
        // Verify agent exists and is accessible in this tenant
        let agent = ctx
            .ctx
            .repos()
            .agents
...
/// error reply. The agent's visibility is verified by the caller's lookup, so
/// the recorder's "not visible" branch is unreachable here.
async fn record_slash_selection(ctx: &PlatformCommandContext, agent_id: &str) {
    if let Err(e) = record_agent_selection(
        ctx.ctx.repos().agents.as_ref(),
        agent_id,
        ctx.user_id,
        ctx.tenant_id,
        AgentSelectionSource::SlashCommand,
    )
    .await
    {
        warn!(agent_id, error = %e, "failed to record coach usage from slash command");
    }
}

/// Point a group at `agent_id` and record the selection.
///
