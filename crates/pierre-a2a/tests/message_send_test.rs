...
// ABOUTME: Verifies tool-intent extraction from data parts and unauthenticated rejection
//
...
//!
//! `SendMessage` is authenticated and forwards real work: a tool invocation
//! through the shared registry path, addressed by a `data` part carrying
//! `tool_name`/`parameters`. A message that addresses no registered tool is
//! refused with the spec's `ContentTypeNotSupported` error.
