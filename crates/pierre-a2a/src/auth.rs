...
use crate::client::{A2AClientManager, ClientRegistrationRequest};
...
pub struct A2AAuthenticator {
    /// Narrow runtime-context slice: auth manager, JWKS, repos for JWT validation
    /// and A2A client / tenant lookups.
    ctx: Arc<dyn A2ACtx>,
    /// MCP auth middleware — used for API-key authentication flow that
    /// delegates to the shared API-key validation pipeline.
    auth_middleware: Arc<McpAuthMiddleware>,
    /// A2A client manager — used for per-client rate-limit lookups and
    /// for registering new clients during `OAuth2` onboarding. Held as a
    /// concrete `pierre-a2a` type rather than on `A2ACtx` to avoid a
    /// `pierre-runtime-context` ↔ `pierre-a2a` dependency cycle.
...
impl A2AAuthenticator {
    ...
    pub async fn authenticate_api_key(&self, api_key: &str) -> AppResult<AuthResult> {
        ...
        let middleware = &self.auth_middleware;
