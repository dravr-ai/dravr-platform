// ABOUTME: The coach seeder's package pass — stores the flavour, skeleton and workouts a coach directory ships beside its prompt
// ABOUTME: A package is replaced as a set per coach on every run; a directory with no package files clears what the coach carried
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! # Coach package artefacts, seeded
//!
//! Pass 1 of [`super::coaches`] writes the coach row; this pass follows it
//! with the package the same directory carries — `flavour.yaml`,
//! `skeleton.yaml`, `workouts/*.toml` — read through
//! [`pierre_coach_parser::read_package_artefacts`], which runs each file
...
//! next seed the way a retired coach does.
...
use glob::glob;
use pierre_coach_parser::{has_package_files, read_package_artefacts};
...
/// Store the package of every seeded coach.
///
/// `slug_to_id` is Pass 1's map of the coaches that have a row; a coach that
...
pub async fn sync_packages<S: BuildHasher + Sync>(
    ...
    for slug in slugs {
        ...
        let Some(dir) = coach_dir(coaches_dir, slug) else {
            continue;
        };
        sync_one(repos, &dir, slug, coach_id, tenant_id, dry_run, &mut stats).await;
    }
    info!(
...
/// Read one coach's package and replace its rows.
///
/// A directory with no package files still writes — an empty set — when the
/// coach carried rows before, so a package deleted from the checkout is
...
async fn sync_one(
    ...
    if packageless && (dry_run || !carries_rows(repos, tenant_id, coach_id).await) {
        return;
    }
    if dry_run {
    ...
    match repos
        .coach_artefacts
        .replace_coach_artefacts(tenant_id, coach_id, &artefacts)
        .await
    {
        Ok(count) => {
...
async fn carries_rows(repos: &RepositoryRegistry, tenant_id: &str, coach_id: &str) -> bool {
    match repos
        .coach_artefacts
        .list_coach_artefacts(tenant_id, coach_id)
        .await
    {
        Ok(rows) => !rows.is_empty(),
        Err(e) => {
            warn!(coach_id, error = %e, "could not read the coach's stored package");
            false
        }
    }
}
...
/// The coach's directory: `coaches_dir/<category>/<slug>`, whatever the
/// category directory is called — the layout the prompt scan walks.
fn coach_dir(coaches_dir: &Path, slug: &str) -> Option<PathBuf> {
    let pattern = coaches_dir.join("*").join(slug);
