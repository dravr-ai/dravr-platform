// ABOUTME: Pure drift-classification logic shared by pierre-cli check-drift agents
// ABOUTME: I/O-free comparison primitives so unit tests can drive every branch without a DB
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! # Agent Drift Classification
//!
//! Pure functions and value types consumed by the `pierre-cli check-drift
//! agents` binary subcommand. Lives in the library (not the binary) so the
...
//!
//! Background: on 2026-05-25 we discovered the prod `dravr-mcp-server-seed-agents`
...
/// Snapshot of the `(source, content_hash)` columns for one agent row.
///
...
pub struct DbAgentRow {
    /// The `agents.source` column value: `'contremaitre'`, `'seed'`, or `'custom'`.
    pub source: String,
    /// The `agents.content_hash` column value, NULL on pre-2026-04 rows.
...
pub enum DriftOutcome {
    ...
    /// for that agent yet.
    SourceMismatch {
        /// The unexpected `agents.source` value.
...
/// not an error.
#[must_use]
pub fn classify_drift(file_hash: &str, db_row: Option<&DbAgentRow>) -> DriftOutcome {
