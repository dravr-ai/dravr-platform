// ABOUTME: A coach package's training artefacts — the flavour, skeleton and workout files shipped beside a coach's prompt
// ABOUTME: Stored as the source text the kernel validated, keyed by (coach, kind, slug); parsed back into kernel types on read
//
...
//! Dravr catalogue in contremaitre's `training/` tree, shared by every coach;
//! the third is an athlete's own `workout_templates` row. Between them sits
//! the coach package: a `flavour.yaml`, a `skeleton.yaml` and `workouts/*.toml`
//! laid beside the coach's `<locale>.md`, parsed by the same kernel the
//! catalogue is, and stored on the coach row they belong to.
//!
//! A package's flavour is its *house flavour*: the one the selection rule
//! pins for that coach's athletes when they can run it, and the one that
//! overrides a catalogue flavour of the same id. Resolution is by slug,
//! package over catalogue over compiled-in, and only a coach whose store
...
pub enum ArtefactKind {
    ...
    Flavour,
    /// `skeleton.yaml` — the coach's season skeleton.
    Skeleton,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CoachArtefact {
    /// Row id.
    pub id: String,
    /// The coach row the package belongs to.
    pub coach_id: String,
    /// The coach's tenant, copied so every query carries it.
