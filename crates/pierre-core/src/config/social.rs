// ABOUTME: Social insights configuration types for coach-mediated sharing features
...
//! These types are used for serializing/deserializing social insights configuration
...
impl SocialInsightsConfig {
    ...
    pub fn validate(&self) -> AppResult<()> {
        ...
        if self.streak_config.lookback_days == 0 {
            return Err(AppError::invalid_input("streak lookback_days must be > 0"));
        }
        ...
        if self.activity_fetch_limits.insight_context_limit == 0 {
            return Err(AppError::invalid_input("insight_context_limit must be > 0"));
        }
        ...
        if self.activity_fetch_limits.max_client_limit == 0 {
            return Err(AppError::invalid_input("max_client_limit must be > 0"));
        }
