...
impl AdminConfigRepository for PostgresAdminConfigManager {
    ...
    async fn set_override(&self, params: SetOverrideParams<'_>) -> AppResult<ConfigOverride> {
        ...
        if let Some(tid) = tenant_id {
            ...
            // A system-wide row stores tenant_id as NULL, and PostgreSQL's
            // default NULLS DISTINCT means `UNIQUE(category, config_key,
            // tenant_id)` can never arbitrate this INSERT. The conflict target
            // is instead the partial unique index over the tenant-less rows
            // (`idx_admin_config_overrides_global_unique`), whose predicate must
            // be restated here for index inference to select it.
            sqlx::query(
                r"
                INSERT INTO admin_config_overrides
                    (id, category, config_key, config_value, data_type, tenant_id, created_by, created_at, updated_at, reason)
