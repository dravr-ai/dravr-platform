...
impl PostgresAdminConfigManager {
    ...
    fn row_to_override(row: &PgRow) -> ConfigOverride {
        ...
        // user_id is a UUID column (tenant_id next to it is TEXT) — decode it
        // as one and render to string, never the reverse.
...
impl AdminConfigRepository for PostgresAdminConfigManager {
    ...
    async fn set_override(&self, params: SetOverrideParams<'_>) -> AppResult<ConfigOverride> {
        ...
        // PostgreSQL's default NULLS DISTINCT means `UNIQUE(category,
        // config_key, tenant_id)` can only arbitrate the tenant-scoped INSERT.
        // The system-wide and per-user rows name their partial unique indexes
        // instead, restating each predicate so index inference selects it.
        match scope {
            ConfigScope::Tenant(tid) => {
                sqlx::query(
                    r"
                ...
                VALUES ($1, $2, $3, $4, $5, $6, NULL, $7::uuid, $8, $8, $9)
            ...
            ConfigScope::User(uid) => {
                ...
                INSERT INTO admin_config_overrides
                    (id, category, config_key, config_value, data_type, tenant_id, user_id, created_by, created_at, updated_at, reason)
                VALUES ($1, $2, $3, $4, $5, NULL, $6::uuid, $7::uuid, $8, $8, $9)
            ...
            ConfigScope::Global => {
                ...
                INSERT INTO admin_config_overrides
                    (id, category, config_key, config_value, data_type, tenant_id, user_id, created_by, created_at, updated_at, reason)
                VALUES ($1, $2, $3, $4, $5, NULL, NULL, $6::uuid, $7, $7, $8)
        ...
        let result = match scope {
            ...
            ConfigScope::User(uid) => {
                sqlx::query(
                    r"
                DELETE FROM admin_config_overrides
                WHERE category = $1 AND user_id = $2::uuid
                ",
                )
                .bind(category)
                .bind(uid)
    ...
    async fn log_change(&self, params: LogChangeParams<'_>) -> AppResult<String> {
        ...
        sqlx::query(
            ...
            VALUES ($1, $2, $3::uuid, $4, $5, $6, $7, $8, $9, $10, $11, $12::uuid, $13, $14)
            ",
        )
        .bind(&id)
        .bind(now)
