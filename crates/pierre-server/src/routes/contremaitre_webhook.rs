...
use pierre_contremaitre::config::ContremaitreConfig;
use pierre_contremaitre::errors::ContremaitreError;
use pierre_contremaitre::sync::{selective_sync, ContremaitreRegistries};
...
fn spawn_selective_sync(
    ...
    let registry = Arc::clone(&resources.mcp.prompt_registry);
    let tool_desc_registry = Arc::clone(&resources.mcp.tool_description_registry);
    let evidence_registry = Arc::clone(&resources.mcp.evidence_registry);
...
async fn handle_contremaitre_webhook(
    ...
    let Some(config) = resources.mcp.contremaitre_config.as_ref() else {
