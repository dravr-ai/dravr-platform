// ABOUTME: Pins visual_blocks onto the hot-reload prompt registry
// ABOUTME: A synced edit to the document must reach the next chat turn without a redeploy
//
...
//! carnet#312.
//!
//! The contremaitre manifest declares the system prompts and every one of
//! them is meant to reach a running binary through the sync — webhook or poll
//! — in about a minute. `visual_blocks.md` governs the inline-chart contract;
//! it is athlete-visible, and it was read as a compiled-in constant at the
//! point the chat pipeline context is built, so an edit needed a contremaitre
//! rev bump and a full platform redeploy while the manifest advertised it as
//! syncable.
//!
