// ABOUTME: The agent seeder claims legacy `source = 'seed'` rows for the catalogue on every run
...
async fn source_of(repos: &RepositoryRegistry, slug: &str) -> String {
    repos
        .seeder
        .seed_find_agent_drift_info(slug)
        .await
        .unwrap()
...
/// report an agent it does not manage as an orphan every morning.
#[tokio::test]
