// ABOUTME: Measures the workspace and asserts AGENTS.md states the same numbers
// ABOUTME: Guards the counts and provider list that drifted for months because nothing read them
//
...
//! literal: the test counts the crates, the server integration test files and
//! the provider features `server-production` compiles, then asserts AGENTS.md
//! repeats what it measured. Adding a crate therefore fails this test until the
//! doc follows, which is the whole point — the numbers it replaces were off by
//! 3.4x (14 vs 48 crates) and 1.7x (325 vs the real server test file count).
//!
