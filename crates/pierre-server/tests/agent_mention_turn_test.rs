// ABOUTME: A `@handle` mention hands one turn to an installed agent and nothing after it
...
//! `@recovery-agent` turn is assembled from the recovery agent's persona, the
//! next plain turn from the conversation's own agent, and the conversation row
...
/// An agent the athlete authored themselves — the kind a conversation is bound
...
fn stripping_removes_only_the_resolved_token_and_closes_the_gap() {
    let recovery = AgentHandle::parse("recovery-coach").unwrap();
    assert_eq!(
...
/// The `ultracode` shape, end to end on the web ladder: the mentioned agent's
...
/// written, and the next plain turn is the conversation's own agent again.
#[tokio::test]
#[serial]
async fn a_mention_routes_that_turn_only_and_the_next_turn_reverts() {
    ...
    .await;
    let installed = install_catalogue_agent(repos, origin, athlete_id, athlete_tenant).await;
    assert_eq!(installed.handle.as_deref(), Some("recovery-coach"));

    let tempo_id = create_custom_agent(
        &resources,
        athlete_id,
        athlete_tenant,
        "Tempo Coach",
    ...
    assert_eq!(
        conversation_agent(&resources, &conversation.id, athlete_id, athlete_tenant)
            .await
            .as_deref(),
...
/// A mention resolves only against installed agents: a catalogue agent the
/// athlete never installed does not route, and neither does a handle nobody
/// owns. The turn proceeds with its text untouched.
#[tokio::test]
#[serial]
async fn a_mention_of_an_agent_that_is_not_installed_does_not_route() {
    env::set_var("PIERRE_LLM_MODEL", "mock-model");
    let mock = CapturingLlm::new();
    let log = mock.log();
    let resources = create_test_server_resources_with_llm(Arc::new(mock))
    ...
    // Published, so `strength-agent` exists in the catalogue — but the athlete
    // never installed it.
...
mod messaging {
    ...
    async fn a_mention_from_telegram_routes_exactly_like_web() {
        ...
        .await;
        let installed = install_catalogue_agent(repos, origin, athlete_id, tenant_id).await;
        assert_eq!(installed.handle.as_deref(), Some("recovery-coach"));

        // The conversation's own agent, selected the way `/agent add` selects
        ...
        repos
            .agents
            .activate_agent(&tempo_id, athlete_id, tenant_id)
            .await
            .unwrap()
        ...
        assert_eq!(
            conversation_agent(&resources, &conversation_id, athlete_id, tenant_id)
                .await
                .as_deref(),
            Some(tempo_id.as_str()),
        ...
        // Turn 2 — plain: the conversation's own agent again, same row.
