// ABOUTME: Coach package artefacts — parsed from the coach directory, stored per coach, lent to a plan only when published
...
//! directory laid out like a contremaitre coach, the rows the seeder writes,
//! the `PublishStatus` gate in `load_coach_package`, the shadowing rules of
...
use pierre_coach_parser::read_package_artefacts;
...
use pierre_services::coach_package::{
    load_coach_package, review_package, CatalogueTier, PackagedCatalogue,
};
...
/// A coach directory laid out like contremaitre's: `en.md` beside the package.
...
/// A published coach carrying `flavour_yaml` as its one flavour, plus one
...
async fn published_coach_with_package(
    ...
    let stored = repos
        .coach_artefacts
        .replace_coach_artefacts(&tenant.to_string(), &coach_id.to_string(), &artefacts)
        .await
        .unwrap();
    assert_eq!(stored, 3);
    coach_id
...
async fn the_repository_replaces_the_set_and_lists_it_in_order() -> Result<()> {
    ...
    let rows = repos
        .coach_artefacts
        .list_coach_artefacts(&tenant.to_string(), &coach_id.to_string())
        .await?;
    ...
    repos
        .coach_artefacts
        .replace_coach_artefacts(&tenant.to_string(), &coach_id.to_string(), &only_flavour)
        .await?;
    let rows = repos
        .coach_artefacts
        .list_coach_artefacts(&tenant.to_string(), &coach_id.to_string())
    ...
    repos
        .coach_artefacts
        .replace_coach_artefacts(&tenant.to_string(), &coach_id.to_string(), &[])
        .await?;
    assert!(repos
        .coach_artefacts
        .list_coach_artefacts(&tenant.to_string(), &coach_id.to_string())
...
async fn a_package_is_lent_only_while_its_listing_is_published() -> Result<()> {
    ...
    let package = load_coach_package(repos, tenant, user_id, Some(&id))
        .await?
    ...
    repos.store_listings.unpublish_coach(&id, tenant).await?;
    assert!(load_coach_package(repos, tenant, user_id, Some(&id))
        .await?
        .is_none());

    // No coach at all: nothing, not an error.
    assert!(load_coach_package(repos, tenant, user_id, None)
        .await?
        .is_none());
    Ok(())
}
...
async fn an_installed_copy_reads_the_origin_package() -> Result<()> {
    ...
    let copy =
        helpers::coach_fixtures::install_catalogue_coach(repos, origin, author, tenant).await;
    assert_eq!(copy.forked_from, Some(origin));

    let package = load_coach_package(repos, tenant, author, Some(&copy.id.to_string()))
        .await?
        .expect("the copy resolves its origin's package");
    assert_eq!(package.coach_id, origin.to_string());
    Ok(())
}
...
async fn package_over_catalogue_over_compiled_in() -> Result<()> {
    ...
    let package = load_coach_package(repos, tenant, user_id, Some(&coach_id.to_string()))
        .await?
        .expect("package");
...
async fn a_saved_day_records_the_tier_its_template_came_from() -> Result<()> {
    ...
    // Unpublish the coach: the house flavour is no longer a flavour the
...
async fn the_rule_pins_the_house_flavour_with_package_provenance() -> Result<()> {
    ...
    // The tool reads the coach from the conversation, never from an
    // argument, so the call runs inside a chat with that coach.
    let conversation = repos
        ...
        .await?;
    assert_eq!(
        conversation.coach_id.as_deref(),
        Some(coach_id.to_string().as_str()),
...
async fn the_rest_template_list_lays_the_selected_coachs_package_over_the_catalogue() -> Result<()>
{
    ...
    repos
        .tenants
        .set_selected_coach(tenant, user_id, Some(&coach_id.to_string()))
        .await?;
