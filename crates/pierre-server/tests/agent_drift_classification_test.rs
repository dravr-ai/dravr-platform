// ABOUTME: Unit coverage for the agent drift classifier consumed by `pierre-cli check-drift agents`
// ABOUTME: Drives every DriftOutcome branch without touching a database
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai
//
// Background: 2026-05-25 audit revealed seed-agents Cloud Run job had been
...
fn row(source: &str, hash: Option<&str>) -> DbAgentRow {
    DbAgentRow {
...
fn source_mismatch_when_db_source_is_custom_even_if_hashes_match() {
    // Custom agents are user-owned: hash equality is coincidental, not
