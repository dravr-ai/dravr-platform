// ABOUTME: The Agent Store is reachable from a chat turn — browse, search and install as MCP tools
// ABOUTME: Content-asserting: real published agents through the executor, then a real installed copy

// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! carnet#62: the agent store was invisible to every chat surface.
//!
//! `CHAT_CALLABLE_CATEGORIES` named no store category, so the LLM was never
//! shown a tool that could browse or install an agent — on web, on mobile, and
...
//! published agent by title, search finds it by a word from that title, and
...
/// Publish an agent into the Store, exactly as `store_routes_test` does, so the
...
fn store_tools_are_chat_callable() {
    ...
    // absent: removing an agent the athlete has history with is a UI gesture,
...
#[tokio::test]
async fn browse_agent_store_returns_published_agents() -> Result<()> {
    ...
    assert_eq!(payload["agents"][0]["title"], "Ultra Trail Fuelling");
    assert_eq!(payload["agents"][0]["category"], "nutrition");
    assert_eq!(
...
#[tokio::test]
async fn search_agent_store_matches_published_titles() -> Result<()> {
    let executor = create_executor().await?;
    let (user_id, tenant_id) = create_test_user(&executor).await?;
    publish_agent(
        &executor,
    ...
    assert_eq!(hit["agents"][0]["title"], "Ultra Trail Fuelling");
...
#[tokio::test]
async fn install_agent_from_store_creates_the_athletes_own_copy() -> Result<()> {
    let executor = create_executor().await?;
    let (author_id, tenant_id) = create_test_user(&executor).await?;
    let published = publish_agent(
        &executor,
        author_id,
        tenant_id,
        "Ultra Trail Fuelling",
    ...
    assert_eq!(payload["agent"]["title"], "Ultra Trail Fuelling");
    ...
    let copy_coach_id = payload["agent"]["id"]
        .as_str()
    ...
    let installed = executor
        ...
        .repositories()
        .store_listings
        .get_installed_agents(installer_id, tenant_id)
        .await?;
    assert_eq!(
        installed.len(),
        1,
