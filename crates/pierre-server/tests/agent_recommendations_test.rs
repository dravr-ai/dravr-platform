// ABOUTME: Unit tests for the personalized agent recommender (sport profile + scoring)
// ABOUTME: Covers provider gating, sport matching, sport-agnostic agents, and cold start
//
...
use pierre_services::agents::score_agent;
...
fn locked_when_no_provider_connected() {
    // A provider-gated agent is not eligible for a user with zero connected
    // providers (cold start), regardless of any (empty) profile.
    let cfg = AgentRecommendationConfig::default();
...
#[test]
fn cross_provider_running_agent_matches_garmin_user() {
    // An agent whose prerequisite names `strava` must still match a Garmin
...
fn sport_mismatch_not_eligible_even_with_provider() {
    // Pure cyclist should not see a Run agent recommended.
    let cfg = AgentRecommendationConfig::default();
...
#[test]
fn sport_agnostic_agent_eligible_once_provider_connected() {
    // Recovery-style agent: provider-gated, no activity types.
    let cfg = AgentRecommendationConfig::default();
    let cyclist = profile(&[("ride", 10)]);
    let rec = score_agent(
        &prereqs(&["strava"], 5, &[]),
...
fn cold_start_recommends_only_provider_free_starters() {
    ...
    let starter = score_agent(&prereqs(&[], 0, &[]), None, &providers(&[]), &cfg);
    assert!(starter.eligible);
    // Provider- or sport-gated agents wait until we can see activity.
    let gated = score_agent(
        &prereqs(&["strava"], 5, &["Run"]),
        None,
        &providers(&[]),
        &cfg,
...
fn below_min_activities_is_penalized_but_still_eligible() {
    let cfg = AgentRecommendationConfig::default();
    // Runner with only 2 activities; agent asks for 5.
...
/// An agent asking for `Ride` must match the athlete whose rides are logged
...
/// from his agent, this hides the cycling agents from a rider whose provider
/// tags every ride as a discipline.
#[test]
fn a_ride_agent_matches_an_athlete_who_logs_only_mountain_bike_and_gravel() {
    let cfg = AgentRecommendationConfig::default();
...
///
/// An athlete who logs every run as `TrailRunning` matched no `Run` agent at
/// all, which is most trail runners.
#[test]
fn a_run_agent_matches_an_athlete_who_logs_only_trail_running() {
    let cfg = AgentRecommendationConfig::default();
...
#[test]
fn a_discipline_specific_agent_stays_exact() {
    let cfg = AgentRecommendationConfig::default();
...
#[test]
fn a_ride_agent_matches_an_athlete_whose_rides_are_split_across_disciplines() {
    let cfg = AgentRecommendationConfig::default();
...
#[test]
fn a_run_agent_matches_an_athlete_whose_runs_are_split_road_trail_treadmill() {
    let cfg = AgentRecommendationConfig::default();
...
/// The stated intent, unchanged: one cross-training ride surfaces no cycling agent.
///
...
#[test]
fn one_cross_training_ride_in_thirty_still_surfaces_no_cycling_agent() {
    let cfg = AgentRecommendationConfig::default();
...
/// A discipline-specific agent stays exact even when the head was inserted.
///
...
/// `MountainBike` agent must still not match through it.
#[test]
fn an_inserted_family_head_does_not_make_a_specialist_agent_match() {
    let cfg = AgentRecommendationConfig::default();
    let three_bikes = profile(&[
        ("run", 20),
        ("walk", 4),
...
#[test]
fn overlap_normalizes_titlecase_agent_activity_types() {
    let cfg = AgentRecommendationConfig::default();
    let runner = profile(&[("run", 10)]);
    // Agent frontmatter uses TitleCase "Run"; user activities are snake_case "run".
