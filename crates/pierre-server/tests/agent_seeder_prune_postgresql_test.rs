// ABOUTME: PostgreSQL-lane test for the agent seeder's prune pass — the listing query is a separate statement
// ABOUTME: A tenant_id cast or a boolean literal wrong on PG hides on SQLite, so the deletion is proven on both
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! `PostgreSQL` agent-seeder prune test.
//
...
/// boolean — both PG-only spellings. Prove the retired agent goes, the kept one
/// stays, and an operator-authored system agent in the same tenant is untouched.
#[tokio::test]
async fn test_pg_retired_catalogue_agent_is_deleted_and_others_survive() {
...
///
/// A merged agent hands its conversation and group to the successor, and a
/// group bound to an agent with no successor blocks the delete rather than
/// dangling.
#[tokio::test]
async fn test_pg_merged_agent_hands_over_and_orphan_group_blocks() {
    let db = create_test_db().await.unwrap();
    let repos = db.repositories();
    bootstrap::run(
        BootstrapArgs {
    ...
    let conversation = repos
        ...
        .id;
    let now = Utc::now();
    let group_for = |agent: &str| CoachingGroup {
        id: Uuid::new_v4(),
        tenant_id: tenant.to_string(),
        name: format!("group of {agent}"),
        description: None,
