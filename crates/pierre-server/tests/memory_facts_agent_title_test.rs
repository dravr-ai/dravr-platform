...
// ABOUTME: GET /api/memory/facts names the agent behind a fact by title, never by its raw id alone
// ABOUTME: One lookup per distinct agent; a fact with no agent, or an agent that no longer resolves, carries no title
...
#[tokio::test]
async fn facts_carry_the_agent_title_when_the_agent_resolves() {
    ...
    for (agent, object) in [
        (Some(agent_id.as_str()), "a sub-4 marathon"),
    ...
    assert_eq!(coached["agent_id"], agent_id.as_str());
    assert_eq!(
        coached["agent_title"], "Marathon Coach",
    ...
    assert!(uncoached["agent_id"].is_null());
    assert!(
        uncoached["agent_title"].is_null(),
