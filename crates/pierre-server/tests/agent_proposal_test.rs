// ABOUTME: Unit tests for the agent-proposal re-rank prompt builder and response parser
// ABOUTME: Covers JSON extraction tolerance, id validation, dedup, capping, and prompt contents
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! Tests for `pierre_services::agents` re-ranking helpers (pure functions: no
//! LLM, no DB) plus the messaging agent-proposal idempotency flag round-trip
...
#[tokio::test]
async fn agent_proposal_flag_round_trips() {
    ...
    assert!(
        !repos
            .messaging
            .agent_proposal_sent(tenant_id, channel, channel_user_id)
            .await
            .unwrap(),
    ...
    repos
        .messaging
        .mark_agent_proposal_sent(tenant_id, channel, channel_user_id, &[])
        .await
        .unwrap();
    ...
    assert!(
        !repos
            .messaging
            .agent_proposal_sent(tenant_id, channel, "tg-other")
            .await
            .unwrap(),
...
/// Write a link's `linked_at` and `agent_proposal_sent_at` directly: the
...
/// The column the backfill names was renamed by 20260909000002, and an applied
/// migration cannot be edited to follow it. So the shipped statement is replayed
/// against today's schema with exactly that one rename applied — still the
/// migration's own text rather than a paraphrase, which is what makes this test
/// worth having.
fn as_shipped_against_head(sql: &str) -> String {
    sql.replace("coach_proposal_sent_at", "agent_proposal_sent_at")
}
...
async fn backfill_stamps_only_links_predating_the_feature() {
    ...
    // (id, linked_at, agent_proposal_sent_at)
    let rows = [
