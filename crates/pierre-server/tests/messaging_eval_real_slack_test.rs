...
use helpers::messaging_eval::assert_guardrail;
use reqwest::Client;
...
async fn real_slack_scope_refusal_e2e() {
    ...
    let Some(reply) = wait_for_coach_reply(&client, &creds, &post_ts, 30).await else {
        panic!(
            "No reply from coach bot ({coach}) within 30s of post at ts={post_ts}. \
             First thing to check: is SLACK_ALLOWED_BOT_IDS set on the running \
             Pierre server? It must include the QA driver bot's `bot_id` (from \
             `auth.test`, not user_id). Without the allow-list, canot drops \
             bot-authored Slack messages before the pipeline ever sees them.",
            coach = creds.coach_user_id,
        );
    };

    eprintln!("Coach replied: {reply}");
    assert_guardrail(&reply, "outside what I can help with")
        .expect("coach reply must carry the scope-refusal substring");
}
