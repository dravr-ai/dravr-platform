// ABOUTME: An agent picked mid-session must reach the conversation the athlete is already in
...
//! A messaging channel holds ONE long-lived conversation per athlete. The agent
//! was bound only when that conversation was created, so activating a different
//! one changed nothing the athlete could see — the previous agent kept
//! answering until `/reset` forged a fresh conversation, which is why `/reset`
//! became the way to change agent, at the cost of the whole thread.
//!
...
//! activation moves `tenant_users.selected_agent_id`, turn two must carry the
//! new agent on the SAME conversation row.

mod common;
mod helpers;

#[cfg(feature = "client-messaging")]
mod agent_rebind_tests {
    ...
    #[tokio::test]
    #[serial]
    async fn activating_an_agent_rebinds_the_live_messaging_conversation() {
        ...
        // Turn 1 — opens the session and its one conversation, with no agent
        // selected yet.
        ...
        assert_eq!(
            conversation_agent(&resources, &conversation_id).await,
            None,
            "fixture precondition: no coach selected before the activation"
        );

        // The athlete picks an agent — exactly what /agents does.
        let agent = resources
            .common
            .repos
            .agents
        ...
        // Turn 2 — same session, same conversation, new agent.
        let router = MessagingRoutes::routes(Arc::clone(&resources));
        let resp = AxumTestRequest::post("/api/messaging/webhook/telegram")
            .header("content-type", "application/json")
            .header("x-telegram-bot-api-secret-token", tg_secret)
        ...
        // And it now belongs to the agent the athlete picked. Before the fix this
