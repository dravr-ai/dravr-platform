...
async fn test_jwks_endpoint_format() -> Result<()> {
    // Read-only: the shared fixture key is generated by the same call at the
    // same size, so the JWKS document it produces is the one under test.
...
async fn test_rs256_user_session_tokens() -> Result<()> {
    // Read-only: signs and validates against one key set, never rotates it.
    let jwks_manager_arc = common::get_shared_test_jwks();
...
async fn test_key_rotation_with_active_tokens() -> Result<()> {
    // The signing manager is only read here; the rotation under test happens
    // on the second manager built below.
    let jwks_manager_arc = common::get_shared_test_jwks();
    let auth_manager = AuthManager::new(24);
    ...
    let new_kid = {
        ...
        // The size is named for the same reason every other keygen in this
        // file names it: `generate_rsa_key_pair` defaults to RSA_KEY_SIZE,
        // which is 4096. Only the returned kid is read here, so the seeded
        // key's size decides nothing but how long the test takes.
        let mut new_manager = JwksManager::new();
        new_manager.generate_rsa_key_pair_with_size(&kid_before, 2048)?;
...
async fn test_rs256_tampered_token_rejection() -> Result<()> {
    // Read-only: the property is the signature check, not the key's origin.
    let jwks_manager_arc = common::get_shared_test_jwks();
    let auth_manager = AuthManager::new(24);
...
async fn test_rs256_wrong_jwks_rejection() -> Result<()> {
    // Only the VALIDATING side has to be a key set of its own — that mismatch
    // is the property under test. The signing side is read-only.
    let jwks_manager1_arc = common::get_shared_test_jwks();
...
async fn test_rs256_admin_token_expiration() -> Result<()> {
    // Read-only: the property is the `exp` claim, not the key's origin.
    let jwks_manager_arc = common::get_shared_test_jwks();
    let jwt_manager = AdminJwtManager::new();
...
async fn test_rs256_unknown_kid_rejection() -> Result<()> {
    // The validating side is read-only; what matters is that it does not hold
    // `unknown_key`.
    let jwks_manager_arc = common::get_shared_test_jwks();
    let auth_manager = AuthManager::new(24);
