...
async fn test_generated_types_match_schemas() -> Result<()> {
    ...
    // packages/ lives at workspace root, not in the crate directory
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .expect("crate must be under crates/ in the workspace");
    let mcp_types_dir = workspace_root
        .join("packages")
        .join("mcp-types")
        .join("src");
    ...
    let reexport_path = workspace_root.join("sdk").join("src").join("types.ts");
