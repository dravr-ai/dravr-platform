// ABOUTME: Pins that binding an agent can never strip the platform contract from the prompt
// ABOUTME: The 2026-07/08 refusals happened because an agent REPLACED pierre_system.md wholesale

// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! A bound agent replaces the persona block. Before the contract was split
...
//!
//! All 52 contremaitre agent prompts restore none of it, so every agent-bound
//! turn ran without them and the agent declined data questions it could have
...
//! contract is present whether or not an agent is bound.
...
fn the_persona_block_carries_voice_and_no_invariants() {
    // The persona is what an agent replaces. If an invariant lives here it is
    // one an agent silently deletes — the whole defect this split fixes. The
    // persona-rules slot itself fell to that exact defect while it lived in
    // pierre_system (agent-bound turns silently lost persona steering), so
