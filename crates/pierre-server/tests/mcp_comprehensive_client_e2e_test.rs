...
impl MockMcpHandler {
    ...
    /// client sees. Tool failures arrive in-band as `isError`.
...
impl McpProtocolTester {
    ...
    async fn test_tool(&mut self, tool_name: &str, arguments: Value) {
        match self.handler.handle_tool_call(tool_name, arguments).await {
            Ok(response) => {
                // A tool failure is the in-band `isError` verdict; its first
                // text block carries the reason.
                if response["result"]["isError"].as_bool() == Some(true) {
                    let error_msg = response["result"]["content"][0]["text"]
                        .as_str()
