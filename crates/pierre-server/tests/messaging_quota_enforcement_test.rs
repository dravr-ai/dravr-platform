// ABOUTME: E2E test that a quota-exhausted athlete's Telegram/WhatsApp turn is refused pre-dispatch
// ABOUTME: with the localized denial reply, counters read under the USER's tenant, never the bot's
...
mod messaging_quota_enforcement_tests {
    ...
    /// turn passes the status and onboarding gates and reaches the turn
    /// service's pre-turn quota check.
    ...
    async fn quota_exhausted_turn_gets_localized_denial_not_silence() {
        ...
        let wa_secret = "wa_quota_enforcement_secret";
