...
/// Returns `(tenant_id, agent_id)` the caller will use for all followup ops.
...
) -> InsertAgentFollowupParams<'a> {
    InsertAgentFollowupParams {
        tenant_id,
        user_id,
        agent_id,
...
async fn tenant_wide_list_orders_by_due_date_nulls_last() -> Result<()> {
    ...
    memory
        .insert_agent_followup(&followup_params(
            tenant,
            "user-a",
            &agent_id,
            "check achilles",
