...
/// # fn example<C: MiddlewareCtx>(resources: Arc<C>) {
/// let app: Router<Arc<C>> = Router::new()
///     .route("/", get(handler))
///     .layer(middleware::from_fn_with_state(resources.clone(), tenant_context_middleware));
/// # }
/// ```
pub async fn tenant_context_middleware<C: MiddlewareCtx>(
...
async fn extract_tenant_from_token<C: MiddlewareCtx>(
    token: &str,
    resources: &Arc<C>,
    ...
    // Resolve tenant ID and build context. The downstream helpers only
    // touch `repos.tenants`, so we hand them the narrower `AuthRepos` view.
    ...
    build_tenant_context(tenant_id, user_id, &repos)
        .await
        .map(|ctx| {
            ctx.with_session_id(claims.guardian_turn_token())
                .with_conversation_id(claims.turn_conversation_id())
        })
}
...
async fn fetch_tenant_name(tenant_id: TenantId, repos: &AuthRepos) -> String {
...
async fn fetch_user_role(user_id: Uuid, tenant_id: TenantId, repos: &AuthRepos) -> TenantRole {
    match repos.tenants.get_user_role(user_id, tenant_id).await {
...
async fn get_user_default_tenant(user_id: Uuid, repos: &AuthRepos) -> Option<TenantId> {
