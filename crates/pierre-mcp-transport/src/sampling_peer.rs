...
use pierre_mcp_schema::{CreateMessageRequest, CreateMessageResult};
...
pub struct SamplingPeer {
    ...
    /// Timeout applied to every outstanding `sampling/createMessage` request
    request_timeout: Duration,
}

impl SamplingPeer {
    /// Create a new sampling peer with access to stdout and a per-request timeout
    ///
    /// `request_timeout` is the maximum time `create_message` will wait for the
    /// client to return a `sampling/createMessage` response before failing the
    /// pending request. Callers typically resolve this from their
    /// `RouteTimeoutConfig::mcp_sampling_timeout_secs` and pass the
    /// `Duration::from_secs` value here.
    #[must_use]
    pub fn new(stdout: Arc<Mutex<Stdout>>, request_timeout: Duration) -> Self {
        Self {
    ...
    pub async fn create_message(
        ...
        // Wait for response with timeout (resolved at construction time by the caller)
