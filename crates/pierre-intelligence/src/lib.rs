// ABOUTME: Fitness intelligence engine with analysis algorithms and data processing
...
// This allows moved files to keep `use crate::errors::*`, `use crate::models::*`, etc.
...
/// Intelligence configuration (recommendations, performance, goals, etc.)
pub mod config;
...
// Re-export social intelligence types (cache + adapter only; validation + insights stay in main)
...
/// Extension trait for `TimeSeriesData` visitor pattern
pub use visitor::TimeSeriesExt;
...
// Re-export fitness profile types from pierre-core (canonical source)
