...
use pierre_core::models::{Activity, InsightType, ShareVisibility, SharedInsight, TrainingPhase};
...
impl SharedInsightGenerator {
    ...
    /// `config` carries the milestone/streak/relevance thresholds; callers in
    /// pierre-server pass `pierre_config::social::global()`.
    #[must_use]
    pub fn generate_suggestions(
        ...
        Self::check_activity_milestones(context, config, &mut suggestions);

        // Check for distance milestones
        Self::check_distance_milestones(context, config, &mut suggestions);

        // Check for streak achievements
        Self::check_streak_achievements(context, config, &mut suggestions);
...
impl InsightContextBuilder {
    ...
    ///
    /// `config` provides the streak lookback window; callers in pierre-server
    /// pass `pierre_config::social::global()`.
    #[must_use]
    pub fn build(self, config: &SocialInsightsConfig) -> InsightGenerationContext {
        ...
        // Calculate streaks
        let (current_streak, longest_streak) = self.calculate_streaks(config);
    ...
    fn calculate_streaks(&self, config: &SocialInsightsConfig) -> (u32, u32) {
