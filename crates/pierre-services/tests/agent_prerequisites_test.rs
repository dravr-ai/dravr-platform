// ABOUTME: An agent's provider prerequisite means "needs activity data", satisfied by any connected provider
...
use pierre_services::agents::check_prerequisites;

fn prereqs(providers: &[&str]) -> AgentPrerequisites {
    AgentPrerequisites {
        providers: providers.iter().map(|p| (*p).to_owned()).collect(),
        ..AgentPrerequisites::default()
    }
}
...
/// Fifteen catalogue agents declare `providers: [strava]`. A Garmin athlete
...
/// An agent with no provider prerequisite needs nothing, connected or not.
#[test]
