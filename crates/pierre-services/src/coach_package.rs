// ABOUTME: Resolves the training catalogue through a coach package — package over catalogue over compiled-in, by slug
// ABOUTME: Loads the package the plan's coach carries when its listing is published, and answers every catalogue read with provenance
//
...
//! [`TrainingCatalogueRegistry`] with the coach's package laid on top. A slug
...
//! it.
//!
//! [`load_coach_package`] is the gate. A package is lent to a plan only when
//! the coach that carries it — the origin, when the plan names an installed
...
//! coaches straight away, which is the same rule with the review done in
//! contremaitre.
//!
...
use pierre_core::models::{ArtefactKind, Coach, CoachArtefact, ParsedArtefact, TenantId};
...
/// A coach's package, parsed: what the resolver lays over the catalogue.
#[derive(Debug, Clone, Default, PartialEq)]
pub struct CoachPackage {
    /// The coach row the artefacts belong to — the origin, never a copy.
    pub coach_id: String,
    /// The house flavour, when the package ships one.
    pub flavour: Option<Flavour>,
    /// The coach's season skeleton, when the package ships one.
...
impl CoachPackage {
    ...
    #[must_use]
    pub fn from_rows(coach_id: &str, rows: &[CoachArtefact]) -> Self {
...
impl<'a> PackagedCatalogue<'a> {
    ...
    /// The registry alone — a read with no coach in hand.
    #[must_use]
    ...
    #[must_use]
    pub const fn package(&self) -> Option<&CoachPackage> {
        self.package.as_ref()
    }
    ...
    #[must_use]
    pub fn house_flavour(&self) -> Option<&str> {
        self.package.as_ref().and_then(CoachPackage::house_flavour)
    }
...
/// Load the package the plan's coach lends its athletes, or `None` when
/// there is no coach, the coach carries no package, or the package is not
/// published.
///
/// `coach_id` is the coach row the plan or conversation names — an
...
/// coach the athlete cannot see resolves nothing.
///
/// # Errors
///
/// Returns the repository error when the coach, its listing or its rows
/// cannot be read.
pub async fn load_coach_package(
    repos: &RepositoryRegistry,
    tenant: TenantId,
    user_id: Uuid,
    coach_id: Option<&str>,
) -> AppResult<Option<CoachPackage>> {
    let Some(coach_id) = coach_id else {
        return Ok(None);
    };
    let Some(origin) = origin_coach(repos, tenant, user_id, coach_id).await? else {
        return Ok(None);
    };
    ...
    let rows = repos
        .coach_artefacts
        .list_coach_artefacts(&origin.tenant_id, &origin_id)
        .await?;
    if rows.is_empty() {
        return Ok(None);
    }
    let package = CoachPackage::from_rows(&origin_id, &rows);
    Ok((!package.is_empty()).then_some(package))
}

/// The coach that authored the package: the row itself, or the row it was
/// forked from when the athlete uses an installed copy.
async fn origin_coach(
    repos: &RepositoryRegistry,
    tenant: TenantId,
    user_id: Uuid,
    coach_id: &str,
) -> AppResult<Option<Coach>> {
    let Some(coach) = repos.coaches.get_by_id(coach_id, user_id, tenant).await? else {
        return Ok(None);
    };
    match coach.forked_from {
        Some(origin) => {
            repos
                .coaches
...
pub struct ArtefactReview {
    ...
    /// kernel admits that only for a grey-practice or coach-judgement tier
...
/// Review a coach's stored artefacts against the catalogue and the evidence
/// corpus.
///
...
pub fn review_package(
    ...
    let package = CoachPackage::from_rows(rows.first().map_or("", |r| r.coach_id.as_str()), rows);
...
fn owner_label(row: &CoachArtefact) -> String {
