// ABOUTME: Agent business logic extracted from route handlers
...
/// agent-proposal REST path) can default to it without a parallel constant.
...
///
/// An agent's `providers` list names the providers it was written against,
/// but what it expresses is "needs activity data": a Garmin runner is as
/// valid for a running agent as a Strava one, and [`score_agent`] has always
...
fn needs_activity_data(prerequisites: &AgentPrerequisites) -> bool {
...
/// Outcome of scoring one agent against a user's profile.
#[derive(Debug, Clone, Copy)]
pub struct AgentRecommendation {
    /// Whether this agent is *eligible* for the "Recommended for you" set
    ...
    /// the recommended set; ineligible agents keep their score for
    /// transparency but stay in the full catalog.
    pub match_score: f32,
}

/// Score a single agent against the user's connected providers and recent
/// sport mix, using the env-tunable [`AgentRecommendationConfig`].
///
/// An agent is only ever eligible when its provider prerequisites are met.
/// Beyond that:
/// - **Sport-specific** agents (non-empty `activity_types`) score by how much
///   the user's active sports overlap the agent's required types.
/// - **Sport-agnostic** agents (empty `activity_types`) help everyone, so once
...
///   provider-free starter agents are eligible — everything sport- or
///   provider-gated waits until we can see what the user actually does.
#[must_use]
pub fn score_agent<S: BuildHasher>(
    prerequisites: &AgentPrerequisites,
    profile: Option<&SportProfile>,
    user_providers: &HashSet<String, S>,
    config: &AgentRecommendationConfig,
) -> AgentRecommendation {
    // Provider gate: an agent that needs a fitness provider requires the user to
    ...
    // sport relevance. An agent with no provider prerequisite (sport-agnostic)
    // is never gated here.
    ...
    match profile {
        ...
        _ => {
            // Cold start: recommend only the broadly-useful agents that need
...
/// One candidate agent reduced to the fields the LLM re-ranking step reads.
///
/// Deliberately excludes the full `system_prompt` (bulk, and irrelevant to
/// *which* agent fits) so the re-rank prompt stays small and on-point — the
/// model ranks on `title` / `category` / `tags` / `blurb` only.
#[derive(Debug, Clone)]
pub struct ProposalCandidate {
    /// Agent id (slug) the model must echo back to select this agent.
    pub id: String,
    /// Agent display title.
    pub title: String,
    /// Agent category label (e.g. `training`, `recovery`).
    pub category: String,
    /// Free-form tags.
    pub tags: Vec<String>,
...
/// One agent the re-ranking step selected, in priority order.
#[derive(Debug, Clone)]
pub struct RankedSelection {
    /// Agent id echoed from the candidate list.
    pub id: String,
    /// One-sentence, second-person rationale for surfacing this agent.
    pub reason: String,
}
...
/// Deterministic rationale localized to `locale` for a sport-matched agent.
///
...
/// Build the user-message body for the agent re-ranking LLM call.
///
...
/// Assign an agent to multiple users after verifying tenant membership
///
...
pub async fn bulk_assign_agent<DB: TenantRepository + ?Sized>(
    manager: &dyn AgentsRepository,
    database: &DB,
    agent_id: &str,
    tenant_id: TenantId,
    admin_user_id: Uuid,
...
/// Unassign an agent from multiple users after verifying tenant membership
///
/// Each target user is validated to belong to the specified tenant before
...
/// Returns error if any user ID is invalid, any user doesn't belong to the tenant,
/// or any database operation fails
pub async fn bulk_unassign_agent<DB: TenantRepository + ?Sized>(
    manager: &dyn AgentsRepository,
    database: &DB,
    agent_id: &str,
    tenant_id: TenantId,
