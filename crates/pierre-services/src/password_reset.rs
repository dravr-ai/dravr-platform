// ABOUTME: Selector/verifier password-reset token generation + parsing (T3MP3ST F1)
// ABOUTME: Delivered token is "<selector>.<verifier>"; only the verifier's SHA-256 hash is stored
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! Password-reset token helpers shared by the self-service and admin-issued reset flows.
//!
//! A reset token is `<selector>.<verifier>`: the plaintext `selector` indexes the token
...
/// A freshly generated reset token: what to persist plus what to deliver.
...
#[must_use]
pub fn split_reset_token(token: &str) -> Option<(String, String)> {
