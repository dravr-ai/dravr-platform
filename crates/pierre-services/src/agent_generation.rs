// ABOUTME: Agent generation from a conversation — the excerpt /agent create drafts from and the persona the model proposes
// ABOUTME: Reads the transcript through the chat repository, asks the LLM for a persona, and reads the per-user agent quota
//
// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2026 dravr.ai

//! Drafting an agent out of the conversation `/agent create` is typed in.
//!
...
//! uses, on either database engine. Only what the athlete and the agent
...
//!
//! Creation itself goes through `AgentsRepository::create` under the same
...
/// Admin-config key for the per-user agent cap.
pub const MAX_AGENTS_PER_USER_KEY: &str = "usage_quotas.max_coaches_per_user";

/// Agent cap applied when the key carries no value and when no admin config
/// is wired at all — the quota degrades to this rather than being skipped.
pub const DEFAULT_MAX_AGENTS_PER_USER: i64 = 3;
...
pub struct AgentProposal {
    ...
    pub title: String,
    /// One-paragraph description of what the agent is for.
    pub description: String,
    /// System prompt that shapes the agent's answers.
...
/// from — the model is never asked to invent an agent out of silence.
///
/// # Errors
///
...
/// Resolve the chat provider an agent proposal runs on.
///
...
/// Where a user stands against their agent cap.
...
impl AgentQuota {
    /// Whether creating one more agent would breach the cap.
    #[must_use]
...
/// Read the user's agent count against `usage_quotas.max_coaches_per_user`.
///
/// One read for `POST /api/agents` and `/agent create confirm`, so the two
...
pub async fn agent_quota(
    ...
    let max = match admin_config {
        Some(config) => config
            ...
            .unwrap_or(DEFAULT_MAX_AGENTS_PER_USER),
        None => DEFAULT_MAX_AGENTS_PER_USER,
    };
    let current = i64::from(agents.count(user_id, tenant_id).await?);
    Ok(AgentQuota { current, max })
}
