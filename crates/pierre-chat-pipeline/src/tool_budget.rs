...
// ABOUTME: Agent column, then admin config, then the compiled-in default — each clamped to the band
...
//! budget outright, the agent's own `max_tool_iterations` column, the
...
///
/// Both sources are range-checked where they are written — the agent column by
/// the agent create/update route, the admin parameter by the admin config
...
pub async fn resolve_max_iterations(
    ...
    if let Some(iterations) = agent_ctx.and_then(|c| c.max_tool_iterations) {
