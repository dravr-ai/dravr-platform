...
pub struct TurnInput {
    ...
    /// The reply's audience: `true` when the athlete is alone with the agent.
    ///
    ...
    /// The agent the athlete addressed by `@handle` in this message, when one
    /// resolved against their installed agents.
    ///
    ...
    /// `agent_id` is never written — the next plain message reverts to the
    /// conversation's own agent. Resolved once by
    /// [`crate::turn_service::execute`] so every surface routes the same way.
    ///
    /// Boxed because it carries the agent's whole runtime context: inline, it
    ...
    pub mentioned_agent: Option<Box<MentionedAgent>>,
}
...
impl TurnInput {
    ...
    ///
    /// Every per-agent read on the turn path goes through here, so a routed
    /// turn is consistently the mentioned agent's — prompt, memory scopes and
    /// attribution alike — rather than a persona swap on top of another
    /// agent's context.
    #[must_use]
    pub fn turn_agent_id<'a>(&'a self, conv: &'a ConversationRecord) -> Option<&'a str> {
        self.mentioned_agent
            .as_ref()
            .map_or(conv.agent_id.as_deref(), |mention| {
                Some(mention.agent_id.as_str())
            })
    }
}
