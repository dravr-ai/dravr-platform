...
//! # Pierre Coach Parser
//!
//! Parses coach definitions authored as markdown files with YAML frontmatter
//! (Claude-Skills-style). Two consumers:
//!
//! - **pierre-server's seeders** at startup, ingesting the on-disk coach
//!   catalogue from `coaches/*.md` into the database.
//! - **pierre-server's `services::coach_import`** when an admin imports a coach
