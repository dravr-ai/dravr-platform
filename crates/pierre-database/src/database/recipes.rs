// ABOUTME: SQLite-backed RecipeRepository, emitted from the shared implementation in repositories/recipes.rs
// ABOUTME: user_id is hyphenated TEXT here, so the shell passes the text uuid codec; search matches with LIKE (ASCII fold)
//
...
impl_recipe_repository!(Database, TextUuid, "LIKE");
