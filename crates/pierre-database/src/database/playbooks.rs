// ABOUTME: SQLite-backed PlaybookRepository for procedural coaching memory
// ABOUTME: Atomic ON CONFLICT counter upsert + tenant-scoped reads; mirrors backends/postgres/playbooks.rs
//
...
/// The `coaching_playbooks` counter upsert. Shared by `record_playbook_outcome`
/// and `record_outcome_and_label` so the atomic `ON CONFLICT` increment is
/// written once.
...
/// Resolve the surviving playbook row's id after the upsert (new id on insert,
...
/// Extract a [`PlaybookRow`] from a `SQLite` row via `try_get` only — keeps
/// corrupt-row handling symmetric with the Postgres impl (a column type/NULL
/// surprise becomes a recoverable error the caller skips, never a panic, which
/// matters because `SQLite` type-affinity can let a non-integer value land in an
/// `INTEGER` column).
fn sqlite_playbook_row(r: &SqliteRow) -> AppResult<PlaybookRow> {
...
/// Extract a [`PendingAdviceRow`] from a `SQLite` row via `try_get` only.
fn sqlite_pending_row(r: &SqliteRow) -> AppResult<PendingAdviceRow> {
...
/// Extract a [`PlaybookAggInput`] from a `SQLite` row via `try_get`.
fn sqlite_agg_row(r: &SqliteRow) -> AppResult<PlaybookAggInput> {
...
/// Extract an [`ArchetypePrior`] from a `SQLite` row via `try_get`.
fn sqlite_prior_from_row(r: &SqliteRow) -> AppResult<ArchetypePrior> {
...
impl PlaybookRepository for Database {
    ...
    async fn list_archetype_priors_for_keys(
        ...
        // final placeholder. Values are still parameterized — only the
        // placeholder list is built dynamically (same idiom as users/recipes).
