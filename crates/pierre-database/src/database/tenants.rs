// ABOUTME: SQLite-backed TenantRepository, emitted from the shared implementation in repositories/tenants.rs
// ABOUTME: uuid columns are TEXT and list columns are JSON text here, so the shared statements take both codecs
//
...
use crate::repositories::tenants::{
    ...
    LIST_TENANTS_FOR_USER_SQL, SET_SELECTED_AGENT_SQL, SET_TENANT_PLAN_SQL,
    STORE_TENANT_OAUTH_CREDENTIALS_SQL,
};
use crate::repositories::uuid_columns::TextUuid;

impl_tenant_repository!(Database, TextUuid, TextList);
