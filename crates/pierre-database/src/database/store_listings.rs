// ABOUTME: SQLite-backed StoreListingsRepository, emitted from the shared implementation in repositories/store_listings.rs
// ABOUTME: user ids are hyphenated TEXT here, so the shell passes the text uuid codec; search matches with LIKE (ASCII fold)
//
...
impl_store_listings_repository!(Database, TextUuid, row_to_agent, "LIKE");
