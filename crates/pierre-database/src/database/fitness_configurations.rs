// ABOUTME: SQLite-backed FitnessConfigRepository, emitted from the shared implementation in repositories/fitness_config.rs
// ABOUTME: user_id is a TEXT column here, so the shared statements bind the user id as its checked text
//
...
impl_fitness_config_repository!(Database, TextUuid);
