...
fn optional_datetime(row: &SqliteRow, column: &str) -> AppResult<Option<DateTime<Utc>>> {
    let raw: Option<String> = row
        .try_get(column)
        .map_err(|e| AppError::database(format!("Failed to read {column}: {e}")))?;
...
fn row_to_compaction_block(row: &SqliteRow) -> AppResult<CompactionBlock> {
...
fn row_to_user_fact(row: &SqliteRow) -> AppResult<UserFact> {
...
// ============================================================================
// HarnessMemoryRepository — SQLite implementation on `Database`
// ============================================================================

#[async_trait]
impl HarnessMemoryRepository for Database {
    ...
    async fn count_user_facts_metrics(&self, tenant_id: TenantId) -> AppResult<UserFactMetrics> {
        ...
        // `COUNT(*)` always returns a non-null integer even when the table is
        // empty. `SUM(CASE ...)` and `MAX(...)` return `NULL` when zero rows
        // match, so those decode as `Option<i64>` / `Option<String>`.
        ...
        let newest_raw: Option<String> = aggregates
            .try_get("newest_updated_at")
            .map_err(|e| AppError::database(format!("Failed to read newest_updated_at: {e}")))?;
        let newest_updated_at = newest_raw.as_deref().map(parse_datetime).transpose()?;
    ...
    async fn insert_coach_followup(
        ...
        let due_at_str = params.due_at.map(|d| d.to_rfc3339());
