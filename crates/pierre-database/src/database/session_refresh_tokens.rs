// ABOUTME: SQLite operations for first-party refresh tokens — store, exchange once, revoke a family or a user
// ABOUTME: Stores the token's HMAC only, through the same blind-index key the OAuth2 server's refresh tokens use
...
#[async_trait]
impl SessionRefreshTokenRepository for Database {
