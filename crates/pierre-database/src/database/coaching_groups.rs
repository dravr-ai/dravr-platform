...
/// Map a `SQLite` row to `GroupTranscriptEntry`
fn row_to_transcript_entry(r: &SqliteRow) -> AppResult<GroupTranscriptEntry> {
    let id_str: String = r.get("id");
    let group_str: String = r.get("group_id");
    let author_str: String = r.get("author_user_id");
    ...
    Ok(GroupTranscriptEntry {
        ...
        group_id: parse_uuid_col("group_id", &group_str)?,
        tenant_id: r.get("tenant_id"),
        author_user_id: parse_uuid_col("author_user_id", &author_str)?,
        speaker: TranscriptSpeaker::from_str_opt(&speaker_str).ok_or_else(|| {
            AppError::database(format!(
...
impl CoachingGroupRepository for Database {
    ...
    async fn list_groups_for_agent(
        ...
        let rows = sqlx::query(
            r"SELECT id, tenant_id, name, description, agent_id, owner_id, coach_user_id,
              ...
              FROM coaching_groups
              WHERE agent_id = $1 AND tenant_id = $2 AND is_active = 1
    ...
    async fn delete_group(&self, group_id: &str, tenant_id: TenantId) -> AppResult<bool> {
        ...
        sqlx::query(r"UPDATE group_invites SET is_active = 0 WHERE group_id = $1")
            .bind(group_id)
            .execute(self.pool())
            .await
    ...
    async fn set_group_coach_user(
        ...
        let now = Utc::now().to_rfc3339();
        let agent = coach_user_id.map(|u| u.to_string());
        let result = sqlx::query(
    ...
    async fn deactivate_invite(&self, group_id: &str, invite_id: &str) -> AppResult<bool> {
        ...
        let result =
            sqlx::query(r"UPDATE group_invites SET is_active = 0 WHERE id = $1 AND group_id = $2")
                .bind(invite_id)
                .bind(group_id)
                .execute(self.pool())
                .await
    ...
    async fn append_transcript_entry(&self, entry: &NewGroupTranscriptEntry<'_>) -> AppResult<()> {
        ...
        .bind(entry.author_user_id.to_string())
