// ABOUTME: SQLite implementation of admin analytics queries for the dashboard
// ABOUTME: Provides tenant-scoped aggregate queries across conversations, coaches, users, and groups
//
...
use super::Database;
use crate::repositories::AdminAnalyticsRepository;

#[async_trait]
impl AdminAnalyticsRepository for Database {
    ...
    async fn count_messages_since(
        ...
        // chat_messages has no tenant_id; join through chat_conversations
        let count: i64 = sqlx::query_scalar(
            r"
    ...
    async fn count_users_with_coach_assignment(&self, tenant_id: TenantId) -> AppResult<i64> {
        // coach_assignments has no tenant_id; join through users -> tenant_users
        let count: i64 = sqlx::query_scalar(
            r"
    ...
    async fn get_daily_active_users_series(
        ...
        // Use jwt_usage joined through users -> tenant_users as activity signal
        let rows = sqlx::query(
            r"
            SELECT date(ju.timestamp) as date, COUNT(DISTINCT ju.user_id) as count
            FROM jwt_usage ju
            INNER JOIN tenant_users tu ON ju.user_id = tu.user_id
            WHERE tu.tenant_id = $1 AND ju.timestamp >= $2
            GROUP BY date(ju.timestamp)
            ORDER BY date ASC
            ",
        )
        .bind(tenant_id.to_string())
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| AppError::database(format!("Failed to get daily active users series: {e}")))?;
    ...
    async fn get_engagement_tiers(&self, tenant_id: TenantId) -> AppResult<EngagementTiers> {
        let row = sqlx::query(
            r"
            SELECT
                COALESCE(SUM(CASE WHEN u.last_active >= datetime('now', '-1 day') THEN 1 ELSE 0 END), 0) as daily_active,
                COALESCE(SUM(CASE WHEN u.last_active >= datetime('now', '-7 days') THEN 1 ELSE 0 END), 0) as weekly_active,
                COALESCE(SUM(CASE WHEN u.last_active >= datetime('now', '-30 days') THEN 1 ELSE 0 END), 0) as monthly_active,
                COALESCE(SUM(CASE WHEN u.last_active < datetime('now', '-30 days') THEN 1 ELSE 0 END), 0) as dormant
            FROM users u
            INNER JOIN tenant_users tu ON u.id = tu.user_id
