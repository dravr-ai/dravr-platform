// ABOUTME: Agent row mappers and hash helpers shared by the SQLite agent repositories
...
use pierre_core::models::agents::{AgentPrerequisites, DataRequirements};
use sqlx::{sqlite::SqliteRow, Row};
use uuid::Uuid;

/// Convert a database row to a `Agent` struct
///
/// # Errors
///
/// Returns an error if row fields cannot be parsed (invalid UUID, datetime, or JSON)
pub fn row_to_agent(row: &SqliteRow) -> AppResult<Agent> {
    ...
    let prerequisites: AgentPrerequisites =
...
/// Convert a database row to an `AgentListItem` (with preference fields from `agent_assignments`)
pub(super) fn row_to_agent_list_item(row: &SqliteRow) -> AppResult<AgentListItem> {
...
/// Convert a database row to an `AgentVersion` struct
///
/// # Errors
///
/// Returns an error if row fields cannot be parsed (invalid UUID, datetime, or JSON)
pub fn row_to_agent_version(row: &SqliteRow) -> AppResult<AgentVersion> {
