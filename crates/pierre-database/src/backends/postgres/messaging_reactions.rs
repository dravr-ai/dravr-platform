...
//!
//! It lives beside the reaction migration rather than inside the 1,200-line
...
pub async fn find_reaction_feedback_target(
    ...
    let row = sqlx::query(
        r"
        SELECT m.chat_message_id, s.tenant_id, s.user_id::text AS user_id, s.channel_user_id,
        ...
        WHERE m.direction = 'outbound'
          ...
          AND ($3::text IS NULL OR COALESCE(s.channel_conversation_id, '') = $3::text)
