// ABOUTME: PostgreSQL-backed SessionRefreshTokenRepository, emitted from the shared implementation in repositories/session_refresh_tokens.rs
// ABOUTME: user_id is a native uuid column here, so the text-bound id is cast ::uuid in and ::text out
...
impl_session_refresh_token_repository!(PostgresDatabase, "::uuid", "::text");
