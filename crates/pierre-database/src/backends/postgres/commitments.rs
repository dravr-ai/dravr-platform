// ABOUTME: Postgres-backed CommitmentRepository for athlete commitments and their swept verdicts
// ABOUTME: Mirrors the SQLite impl statement for statement with try_get (never get — Row::get panics on PG)
//
...
/// Extract a [`CommitmentRow`] from a Postgres row via `try_get` only.
/// `Row::get` is `try_get().unwrap()` and panics the whole read path on a width
/// or NULL surprise, which is how one written sync-cursor row once made every
/// chat refresh panic.
fn pg_commitment_row(r: &PgRow) -> AppResult<CommitmentRow> {
    let col =
...
fn collect_commitments(rows: &[PgRow]) -> Vec<Commitment> {
    rows.iter()
        .filter_map(|r| {
            pg_commitment_row(r)
...
#[async_trait]
impl CommitmentRepository for PostgresDatabase {
