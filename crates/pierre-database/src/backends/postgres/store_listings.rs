// ABOUTME: PostgreSQL implementation of StoreListingsRepository for agent marketplace
...
use super::agents_rows::{row_to_agent_pg, token_count_as_i32};
...
/// Agent columns for non-aliased queries (agents table only)
const AGENT_COLUMNS: &str = r"id, user_id, tenant_id, title, description, system_prompt,
...
/// Agent columns for `JOIN`ed queries with table alias `c`
const AGENT_COLUMNS_ALIASED: &str = r"c.id, c.user_id, c.tenant_id, c.title, c.description, c.system_prompt,
...
/// `PostgreSQL` twin of `database::agent_handle::ensure_catalogue_handle`:
/// give an agent its catalogue handle if it does not own one — on Store
/// approval, and on creation from a conversation.
///
/// An origin agent already carrying a handle keeps it; any other agent gets
/// the first free candidate derived from its title, judged at catalogue
/// scope (no origin agent, no published agent). Origin rows are additionally
/// guarded by the `idx_agents_handle` unique index. Runs inside the
/// approval transaction, or on a plain connection for a created agent.
///
/// # Errors
///
...
/// Convert a `JOIN`ed `PostgreSQL` row to an `AgentWithListing`
///
/// Parses agent fields from `c.*` columns and listing fields from aliased `sl.*` columns.
fn row_to_agent_with_listing_pg(row: &PgRow) -> AppResult<AgentWithListing> {
...
impl PostgresDatabase {
    /// Get an agent with its listing by `agent_id` and `tenant_id` (internal helper)
    async fn get_agent_with_listing_pg(
        &self,
        agent_id: &str,
        tenant_id: &TenantId,
        ...
        row_to_agent_with_listing_pg(&row)
    }
...
impl StoreListingsRepository for PostgresDatabase {
    async fn assign_catalogue_handle(
        ...
        ensure_catalogue_handle_pg(&mut conn, agent_id, tenant_id).await
    }
    ...
    async fn approve_agent(
        ...
        ensure_catalogue_handle_pg(&mut tx, agent_id, tenant_id).await?;
        tx.commit()
            .await
            .map_err(|e| AppError::database(format!("Failed to commit approval: {e}")))?;

        self.get_agent_with_listing_pg(agent_id, &tenant_id).await
    }
    ...
    async fn get_pending_review_agents(
        ...
        rows.iter().map(row_to_agent_with_listing_pg).collect()
    }
    ...
    async fn get_published_agents_cursor(
        ...
        for row in rows {
            all_items.push(row_to_agent_with_listing_pg(&row)?);
        }
    ...
    async fn get_published_agent(&self, agent_id: &str) -> AppResult<Option<AgentWithListing>> {
        ...
        row.map(|r| row_to_agent_with_listing_pg(&r)).transpose()
    }
    ...
    async fn install_from_store(
        ...
        .bind(token_count_as_i32(source.agent.token_count))
        .bind(now)
