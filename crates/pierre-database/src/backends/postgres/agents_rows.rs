// ABOUTME: PostgreSQL row → agent domain mappers plus the agent content/request hashes
...
use pierre_core::models::agents::{
    Agent, AgentCategory, AgentListItem, AgentPrerequisites, AgentVersion, AgentVisibility,
    CreateAgentRequest, DataRequirements,
};
...
pub(super) fn compute_request_hash(request: &CreateAgentRequest) -> String {
    let mut hasher = DefaultHasher::new();
...
/// Convert a `PostgreSQL` row to a `Agent` struct
///
/// Reads PG-native types directly: `UUID` for `user_id`/`tenant_id`, `BOOLEAN`
/// for `is_system`, `TIMESTAMPTZ` for `created_at`/`updated_at`. Agent id is
...
pub(super) fn row_to_agent_pg(row: &PgRow) -> AppResult<Agent> {
    ...
    let sample_prompts: Vec<String> = sample_prompts_json
        ...
        .transpose()?
        .unwrap_or_default();
    let prerequisites: AgentPrerequisites = prerequisites_json
        .as_deref()
        .map(serde_json::from_str)
        .transpose()?
        .unwrap_or_default();
    ...
    Ok(Agent {
        ...
        category: AgentCategory::parse(&category_str),
        tags,
        sample_prompts,
        ...
        updated_at,
        is_system,
        visibility: AgentVisibility::parse(&visibility_str),
        prerequisites,
        forked_from,
        max_tool_iterations,
        temperature,
...
/// Convert a `PostgreSQL` row to an `AgentListItem` (with preference fields from `agent_assignments`)
///
...
pub(super) fn row_to_agent_list_item_pg(row: &PgRow) -> AppResult<AgentListItem> {
...
/// Convert a `PostgreSQL` row to an `AgentVersion` struct
pub(super) fn row_to_agent_version_pg(row: &PgRow) -> AppResult<AgentVersion> {
