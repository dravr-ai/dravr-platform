// ABOUTME: PostgreSQL-backed TrainingPlanRepository — outline + week supersession in transactions
...
fn week_row(row: &PgRow) -> AppResult<PlanWeekRow> {
    Ok(PlanWeekRow {
        id: row.try_get("id").map_err(map_col("id"))?,
        tenant_id: row.try_get("tenant_id").map_err(map_col("tenant_id"))?,
        user_id: row.try_get("user_id").map_err(map_col("user_id"))?,
...
#[async_trait]
impl TrainingPlanRepository for PostgresDatabase {
