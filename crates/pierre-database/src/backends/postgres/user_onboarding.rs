// ABOUTME: PostgreSQL-backed UserOnboardingRepository — durable per-user onboarding step completion state
// ABOUTME: Mirrors the SQLite impl with PG-native binds ($1.., NOW()); all-TEXT columns so no uuid/int decode trap
//
...
/// panics on a live PG connection on a type/NULL surprise) so a corrupt row is a
/// recoverable error. All columns are TEXT, so these are plain `String` decodes —
/// no INT4-as-i64 or native-uuid trap.
fn pg_step_record(row: &PgRow) -> AppResult<OnboardingStepRecord> {
...
#[async_trait]
impl UserOnboardingRepository for PostgresDatabase {
    ...
    async fn get_onboarding_steps(&self, user_id: &str) -> AppResult<Vec<OnboardingStepRecord>> {
        ...
        rows.iter().map(pg_step_record).collect()
    }
}
