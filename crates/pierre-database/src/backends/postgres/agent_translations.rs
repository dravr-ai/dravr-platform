// ABOUTME: The agent_translations overlay reader the Postgres agent and store reads share
// ABOUTME: One query for a page of agent ids; English is canonical and skips the read entirely
//
...
impl PostgresDatabase {
    ...
    pub async fn agent_translation_overlays(
        &self,
        ids: &[String],
        locale: &str,
        ...
        let rows = sqlx::query(
            ...
            WHERE locale = $1 AND agent_id = ANY($2)
            ",
        )
        .bind(locale)
        .bind(ids)
