// ABOUTME: PostgreSQL-backed health persistence (data sources, sleep, recovery, snapshots, sync cursors, time series), emitted from the shared repositories modules
// ABOUTME: user_oauth_tokens.tenant_id is a VARCHAR here while TenantId decodes a native uuid, so the connected-users read casts it
//
...
impl_health_persistence_repositories!(PostgresDatabase);
impl_sync_cursor_repository!(PostgresDatabase, "tenant_id::uuid AS tenant_id");
impl_time_series_store!(PostgresDatabase);
