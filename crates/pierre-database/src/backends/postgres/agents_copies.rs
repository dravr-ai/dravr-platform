// ABOUTME: Per-user copies of catalogue agents on PostgreSQL: forking, and resolving a copy by @handle
...
use pierre_core::models::agents::{Agent, AgentHandle, AgentVisibility};
use pierre_core::models::TenantId;
use sqlx::PgPool;
use uuid::Uuid;

/// Resolve an installed agent by handle for one user — see
/// `AgentsRepository::find_installed_by_handle` for the contract.
pub(super) async fn find_installed_by_handle(
    ...
    row.as_ref().map(row_to_agent_pg).transpose()
}

/// Fork a system agent into the user's own copy, carrying the origin's
/// handle so the copy resolves by the same `@handle`.
pub(super) async fn fork_agent(
    db: &PostgresDatabase,
    source_agent_id: &str,
    user_id: Uuid,
    tenant_id: TenantId,
) -> AppResult<Agent> {
    // Get the source agent (must be a system agent)
    // System agents are platform-wide, so no tenant filter
    let source = db
    ...
    // Serialize data_requirements from source agent for fork INSERT
    ...
    .bind(source_agent_id) // forked_from
