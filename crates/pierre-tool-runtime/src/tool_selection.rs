...
//! Per-tenant tool selection — combines global env-driven disabling, plan
//! restrictions, and admin-configured per-tenant overrides into a single
//! [`EffectiveTool`] list backed by an LRU cache.
...
impl ToolSelectionService {
    ...
    ///
    /// Takes the full `RepositoryRegistry` because the service narrows
    /// internally to two views — `AuthRepos` (plan lookup via
    /// `tenants.get_by_id`) and `UsageRepos` (`tool_selection` reads /
    /// writes). The narrowing is at the field, not the param.
    #[must_use]
    ...
    pub async fn is_tool_enabled(&self, tenant_id: TenantId, tool_name: &str) -> AppResult<bool> {
        ...
        let tenant_plan = match self.auth.tenants.get_by_id(tenant_id).await {
    ...
    pub async fn get_catalog(&self) -> AppResult<Vec<ToolCatalogEntry>> {
        self.usage.tool_selection.get_tool_catalog().await
    }
    ...
    async fn compute_effective_tools(&self, tenant_id: TenantId) -> AppResult<Vec<EffectiveTool>> {
        ...
        let (tenant_plan, override_map) = match self.auth.tenants.get_by_id(tenant_id).await {
            Ok(tenant) => {
                ...
                let overrides: Vec<TenantToolOverride> =
                    self.usage.tool_selection.get_overrides(tenant_id).await?;
        ...
        // Load full catalog
        let catalog: Vec<ToolCatalogEntry> = self.usage.tool_selection.get_tool_catalog().await?;
