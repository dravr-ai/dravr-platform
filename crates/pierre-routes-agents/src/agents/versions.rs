// ABOUTME: Version history route handlers for agent versioning endpoints
// ABOUTME: Provides listing, retrieval, revert, and diff operations on agent version snapshots
//
...
use super::types::{
    AgentDiffResponse, AgentVersionResponse, FieldChange, ListVersionsQuery, ListVersionsResponse,
    RevertVersionResponse,
};

/// Handle GET /api/agents/:id/versions - List version history
pub(super) async fn handle_list_versions<C: AgentsCtx + MiddlewareCtx>(
    ...
    // Authorization: reading version history exposes the agent's private content
    // (title, system_prompt, tags). Mirror the agent-detail access rule — the
    // caller may read versions only if they can access the agent itself
    ...
    // user's private agent via the version endpoints.
    if manager
    ...
    let mut version_responses: Vec<AgentVersionResponse> = Vec::with_capacity(versions.len());
    for version in versions {
        let created_by = version.created_by;
        let mut response: AgentVersionResponse = version.into();
...
pub(super) async fn handle_revert_version<C: AgentsCtx + MiddlewareCtx>(
    State(ctx): State<Arc<C>>,
    auth: AuthenticatedUser,
...
async fn resolve_creator_name<C: AgentsCtx + MiddlewareCtx>(
    ctx: &Arc<C>,
    tenant_id: TenantId,
...
/// for the agents domain: agent version snapshots and recipe snapshots share
