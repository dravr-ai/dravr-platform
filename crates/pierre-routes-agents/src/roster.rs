// ABOUTME: Agent-athlete roster routes — gated by users.manages_roster=true or is_admin=true
...
//!   agent (one row per assigned athlete in the same tenant).
...
//! `users.is_admin=true`. A user with `coaching_persona=Agent` but
//! `manages_roster=false` is rejected — those are independent fields
//! (Agent voice ≠ Agent permission). See
...
use pierre_middleware::extractors::extract_auth_from_headers;
...
/// Build the roster routes router (mount under root — paths are already absolute).
pub fn build_roster_router<C>() -> Router<Arc<C>>
where
    C: AgentsCtx + MiddlewareCtx,
{
    Router::new()
        .route(
            "/api/roster",
            get(list_athletes::<C>).post(assign_athlete::<C>),
        )
        .route(
            "/api/roster/{athlete_user_id}",
            delete(revoke_assignment::<C>),
        )
}
...
async fn require_roster_manager<C: AgentsCtx + MiddlewareCtx>(
    headers: &HeaderMap,
    ctx: &Arc<C>,
) -> AppResult<(User, TenantId)> {
    let auth = extract_auth_from_headers(headers, ctx).await?;
    let users: &dyn UserRepository = ctx.repos().users.as_ref();
    let user = users
...
/// they agent). Empty list when the caller has not been assigned any
/// athletes yet.
async fn list_athletes<C: AgentsCtx + MiddlewareCtx>(
    State(ctx): State<Arc<C>>,
    headers: HeaderMap,
) -> Result<Response, AppError> {
    let (caller, tenant_id) = require_roster_manager(&headers, &ctx).await?;
    let assignments = ctx
        .repos()
        .roster
...
/// has an active assignment from this agent (idempotent).
async fn assign_athlete<C: AgentsCtx + MiddlewareCtx>(
    State(ctx): State<Arc<C>>,
    headers: HeaderMap,
    ...
    let inserted = ctx.repos().roster.assign_athlete(&assignment).await?;
...
async fn revoke_assignment<C: AgentsCtx + MiddlewareCtx>(
    State(ctx): State<Arc<C>>,
    headers: HeaderMap,
