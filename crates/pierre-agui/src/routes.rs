...
//! live stream of [`crate::events::AgUiEvent`]s (one JSON payload per
...
//!    [`crate::registry::RunOwner`] recorded at `register()` time.
...
//! is to use the existing impersonation flow in pierre-server
...
//! ## Feature gating
//!
//! This module only compiles when the `transport-sse` feature is
//! enabled. The route mount in pierre-server's `mcp::multitenant`
//! is gated identically. A pierre-agui build without
//! `transport-sse` still exposes the [`crate::registry::RunRegistry`]
//! for future non-SSE transports (e.g. long-poll, websocket bridge)
//! but no HTTP entry point for AG-UI.

use crate::registry::{AuthorizedSubscribe, RunOwner, RunRegistry, RunSubscription};
use axum::{
    extract::{Path, State},
    http::HeaderMap,
...
use pierre_core::auth_header::extract_bearer_token_owned as extract_token;
...
impl AgUiRoutes {
    ...
    ///
    /// The returned router expects the [`RunRegistry`] plus an
    /// [`Arc<McpAuthMiddleware>`] in its state so the chat pipeline and the
    /// SSE subscribers share the same broadcast channels, and the
    /// handler can authenticate Bearer tokens.
    pub fn routes(registry: RunRegistry, auth: Arc<McpAuthMiddleware>) -> Router {
        Router::new()
            .route(
    ...
    async fn handle_run_stream(
        ...
        headers: HeaderMap,
        State((registry, auth)): State<(RunRegistry, Arc<McpAuthMiddleware>)>,
    ) -> Result<Sse<impl Stream<Item = Result<Event, Infallible>>>, AppError> {
        let caller = authenticate_caller(&headers, &auth).await?;
