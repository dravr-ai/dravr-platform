...
//!   `GET /api/agui/runs/{run_id}/stream` as an SSE endpoint. Only
//!   compiled when the `transport-sse` feature is enabled.
//!
//! ## Pipeline wiring
//!
//! The chat pipeline threads an optional `&dyn AgUiSink` through its
//! hooks. Channel adapters that care about progress feedback construct a
//! [`BroadcastSink`] bound to a fresh `run_id`, pass it in, and hand the
//! `run_id` back to the client so it can open the SSE stream. The sink
//! is silently skipped when absent, so the feature is strictly opt-in
//! per turn.
