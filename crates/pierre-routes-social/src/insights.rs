...
//! Insight handlers.
//!
//! Hosts coach-mediated sharing, LLM-assisted generation/validation,
//! reactions, and per-user adapted insights.
//!
//! The handlers are generic over a state type that satisfies both
//! [`ToolRuntime`] (for the LLM provider, provider registry, tenant OAuth
//! client, notification service, and the two insight prompts) and
//! [`MiddlewareCtx`] (for repository access and the `AuthenticatedUser`
//! extractor). The composition root in `pierre-server` mounts these routes
//! with `Arc<ServerContext>` as the concrete state type.
...
use pierre_auth::config::oauth::default_provider;
...
/// Social-insights route handler.
///
/// Hosts the `/api/social/insights/*`, `/api/social/adapted` (and friends)
/// routes — the subset of the public social surface that needs the LLM
/// provider, provider registry, tenant OAuth client, and notification
/// service from [`ToolRuntime`].
///
/// The friend / feed / settings handlers live in
/// [`super::SocialRestRoutes`]; the composition root merges both routers
/// to expose the full `/api/social/*` surface.
pub struct SocialRoutes;

impl SocialRoutes {
    /// Build the insights + reactions + adapted-insights router.
    ///
    /// The router merges the friend / feed / settings routes from
    /// [`super::SocialRestRoutes`] with the in-crate insights routes so
    /// the public `/api/social/*` surface stays whole. Mounting takes a
    /// state that satisfies [`ToolRuntime`] (for the LLM / provider /
    /// notification surface) and [`MiddlewareCtx`] (for repository access
    /// and the `AuthenticatedUser` extractor).
    pub fn routes<C: ToolRuntime + MiddlewareCtx + pierre_runtime_context::SocialCtx>(
        resources: Arc<C>,
    ) -> axum::Router {
        use axum::routing::{delete, get, post, put};

        let insights_router = axum::Router::new()
            // Insights
            .route("/api/social/insights", get(Self::handle_list_insights::<C>))
            .route(
                "/api/social/insights",
                post(Self::handle_share_insight::<C>),
            )
            .route(
                "/api/social/insights/suggestions",
                get(Self::handle_get_suggestions::<C>),
            )
            .route(
                "/api/social/insights/from-activity",
                post(Self::handle_share_from_activity::<C>),
            )
            .route(
                "/api/social/insights/generate",
                post(Self::handle_generate_insight::<C>),
            )
            .route(
                "/api/social/insights/{id}",
                get(Self::handle_get_insight::<C>),
            )
            .route(
                "/api/social/insights/{id}",
                delete(Self::handle_delete_insight::<C>),
            )
            // Reactions
            .route(
                "/api/social/insights/{id}/reactions",
                get(Self::handle_list_reactions::<C>),
            )
            .route(
                "/api/social/insights/{id}/reactions",
                post(Self::handle_add_reaction::<C>),
            )
            .route(
                "/api/social/insights/{id}/reactions/{reaction_type}",
                delete(Self::handle_remove_reaction::<C>),
            )
            // Adapted insights
            .route(
                "/api/social/insights/{id}/adapt",
                post(Self::handle_adapt_insight::<C>),
            )
            .route("/api/social/adapted", get(Self::handle_list_adapted::<C>))
            .route(
                "/api/social/adapted/{id}/helpful",
                put(Self::handle_update_helpful::<C>),
            )
            .with_state(Arc::clone(&resources));

        super::SocialRestRoutes::routes(resources).merge(insights_router)
    }
    ...
    ///
    /// The trait is implemented for both `SQLite` (`Database`) and
    /// `PostgreSQL` (`PostgresDatabase`); the registry holds whichever
    /// the active backend produces.
    ...
    /// `Arc<dyn ToolRuntime>` for helpers that take the erased handle.
    ...
    ///
    /// Returns the test-injected `llm_provider` trait object when set on the
    /// runtime. **NEVER** falls through to per-call [`ChatProvider::from_env`] —
    /// the per-call fallback was a silent copilot-spawn driver that burned
    /// `ChefFamille`'s GitHub REST budget under any concurrent load (see
    /// [`chat_provider_from_resources_arc`] for the full rationale).
    ///
    /// # Errors
    ///
    /// [`AppError::internal`] when no `llm_provider` is wired on the runtime.
    pub(crate) fn get_llm_provider(
        resources: &Arc<dyn ToolRuntime>,
    ) -> Result<Arc<dyn LlmProvider>, AppError> {
        resources.llm_provider().cloned().ok_or_else(|| {
            AppError::internal(
                "social/insights.get_llm_provider: no llm_provider configured on \
                 runtime. Per-call ChatProvider::from_env() is intentionally \
    ...
    pub(crate) async fn handle_list_insights<C: ToolRuntime + MiddlewareCtx>(
    ...
    pub(crate) async fn handle_share_insight<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
    ...
    pub(crate) async fn handle_get_insight<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        Path(id): Path<String>,
    ) -> Result<Response, AppError> {
    ...
    pub(crate) async fn handle_delete_insight<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        Path(id): Path<String>,
    ) -> Result<Response, AppError> {
    ...
    pub(crate) async fn handle_get_suggestions<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        ...
        .await
        {
            Ok(context) => {
                ...
                let mut suggestions = generator.generate_suggestions(&context, social::global());
    ...
    pub(crate) async fn handle_share_from_activity<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        ...
        let content = if let Some(custom_content) = body.content.clone() {
            ...
            .map_or_else(
                ...
                |context| {
                    let generator = SharedInsightGenerator::new();
                    let suggestions = generator.generate_suggestions(&context, social::global());
        ...
        let validated_content = if body.content.is_some() {
            Self::validate_insight_content(&Self::as_runtime(&resources), &content).await?
        } else {
    ...
    pub(crate) async fn handle_generate_insight<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        _: AuthenticatedUser,
        ...
        // Get LLM provider from resources (injected for tests) or environment
        let llm_provider = Self::get_llm_provider(&Self::as_runtime(&resources))?;
    ...
    pub(crate) async fn handle_list_reactions<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        Path(id): Path<String>,
    ) -> Result<Response, AppError> {
    ...
    pub(crate) async fn handle_add_reaction<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        Path(id): Path<String>,
        ...
        if insight.user_id != auth.user_id {
            if let Some(service) = ToolRuntime::notification_service(&*resources) {
                if let Some(tenant_uuid) = auth.active_tenant_id {
                    let reactor_user = MiddlewareCtx::repos(&*resources)
                        .users
                        .get_global(auth.user_id)
                        .await?;
    ...
    pub(crate) async fn handle_remove_reaction<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
    ...
    pub(crate) async fn handle_adapt_insight<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        Path(id): Path<String>,
    ...
    pub(crate) async fn handle_list_adapted<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
    ...
    pub(crate) async fn handle_update_helpful<C: ToolRuntime + MiddlewareCtx>(
        State(resources): State<Arc<C>>,
        auth: AuthenticatedUser,
        Path(id): Path<String>,
