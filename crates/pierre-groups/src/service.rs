...
impl GroupService {
    ...
    pub async fn inject_group_context(
        ...
        // Broken and stale member states render as trailing alert sections;
        // quiet members need none (see `context_alerts` for the three-state
        // contract). Gated identically to the snapshots: only
        // visible/consenting members were collected above.
        let alerts = connection_and_staleness_alerts(&visible_snapshots);

        Ok(format!("{base_system_prompt}{context_block}{alerts}"))
    }
