...
export default function UserDetailDrawer({
  ...
  const getUsageColor = (percentage: number) => {
    if (percentage >= 90) return 'bg-pierre-red-500';
    if (percentage >= 75) return 'bg-pierre-yellow-500';
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-xl z-50 overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-white border-b border-pierre-gray-200 px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-semibold text-pierre-gray-900">User Details</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <Card className="p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-pierre-gray-500">Registered</span>
                <p className="font-medium text-pierre-gray-900">{formatDate(user.created_at)}</p>
              </div>
              <div>
                <span className="text-pierre-gray-500">Last Active</span>
                <p className="font-medium text-pierre-gray-900">{formatDate(user.last_active)}</p>
              </div>
              {user.approved_at && (
                <div>
                  <span className="text-pierre-gray-500">Approved</span>
                  <p className="font-medium text-pierre-gray-900">{formatDate(user.approved_at)}</p>
                </div>
              )}
              {user.approved_by && (
                <div>
                  <span className="text-pierre-gray-500">Approved By</span>
                  <p className="font-medium text-pierre-gray-900">{user.approved_by}</p>
                </div>
              )}
            </div>
          </Card>
          ...
          {/* Rate Limits Card */}
          <Card className="p-4">
            <h4 className="text-sm font-semibold text-pierre-gray-900 mb-4 flex items-center">
            ...
            {rateLimitLoading ? (
              <div className="space-y-3 animate-pulse">
                <div className="h-4 bg-pierre-gray-200 rounded w-3/4"></div>
                <div className="h-2 bg-pierre-gray-200 rounded w-full"></div>
                <div className="h-4 bg-pierre-gray-200 rounded w-3/4"></div>
                <div className="h-2 bg-pierre-gray-200 rounded w-full"></div>
              </div>
            ) : rateLimit ? (
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-pierre-gray-600">Daily Usage</span>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-pierre-gray-600">Monthly Usage</span>
                    <span className="font-medium">
              ...
              <p className="text-sm text-pierre-gray-500">Unable to load rate limit data</p>
            )}
          </Card>
          ...
          <Card className="p-4">
            ...
            {activityLoading ? (
              <div className="space-y-3 animate-pulse">
                <div className="h-4 bg-pierre-gray-200 rounded w-1/2"></div>
                <div className="h-4 bg-pierre-gray-200 rounded w-3/4"></div>
                <div className="h-4 bg-pierre-gray-200 rounded w-2/3"></div>
              </div>
            ) : activity ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-pierre-gray-600">Total Requests</span>
                  <span className="text-2xl font-bold text-pierre-gray-900">
                ...
                {activity.top_tools.length > 0 ? (
                  <div>
                    <span className="text-sm text-pierre-gray-600">Top Tools</span>
                    <div className="mt-2 space-y-2">
                      {activity.top_tools.slice(0, 5).map((tool) => (
                        <div key={tool.tool_name} className="flex items-center justify-between">
                          <span className="text-sm font-medium text-pierre-gray-900">{tool.tool_name}</span>
                          <div className="flex items-center space-x-2">
                            <span className="text-sm text-pierre-gray-600">{tool.call_count.toLocaleString()}</span>
                            <span className="text-xs text-pierre-gray-400">({tool.percentage.toFixed(1)}%)</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <p className="text-sm text-pierre-gray-500">No tool usage in the last 30 days</p>
                )}
              </div>
            ) : (
              <p className="text-sm text-pierre-gray-500">Unable to load activity data</p>
            )}
          </Card>

          {/* Actions */}
          <Card className="p-4">
            <h4 className="text-sm font-semibold text-pierre-gray-900 mb-4">Actions</h4>
            <div className="space-y-2">
              ...
              {user.user_status === 'pending' && (
                <Button
                  ...
                  className="w-full justify-start bg-pierre-green-600 hover:bg-pierre-green-700 text-white"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {user.user_status === 'active' && (
                <Button
                  ...
                  variant="outline"
                  className="w-full justify-start border-pierre-red-300 text-pierre-red-700 hover:bg-pierre-red-50"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {canImpersonate && (
                <Button
                  ...
                  variant="outline"
                  className="w-full justify-start border-pierre-yellow-300 text-pierre-yellow-700 hover:bg-pierre-yellow-50"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
