...
export default function UserApprovalModal({ 
  ...
  const actionColor = action === 'approve' ? 'bg-pierre-green-600 hover:bg-pierre-green-700' : 'bg-pierre-red-600 hover:bg-pierre-red-700';
          ...
          <Card className="mb-4 p-4 bg-pierre-gray-50">
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col items-end space-y-1">
                ...
                <span className="text-xs text-pierre-gray-500 capitalize">{user.tier}</span>
              </div>
            </div>
          </Card>

          <div className="mb-4">
            <label htmlFor="reason" className="block text-sm font-medium text-pierre-gray-700 mb-2">
              Reason {action === 'approve' ? '(optional)' : '(recommended)'}
            </label>
            <textarea
              id="reason"
              rows={3}
              className="w-full px-3 py-2 border border-pierre-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-pierre-violet focus:border-transparent"
