 ...
 *
 * Three buttons laid over the row covered more than half of it while the
 * pointer was on it, so the right half of every row stopped selecting the
 * conversation. One trigger leaves the row clickable, which is also how
 * Telegram and iMessage reach the same three actions; a right-click on the
    ...
    >
      ...
      {open && (
        <div
          ...
          className="absolute right-0 z-30 mt-1 w-48 rounded-xl border ghost-border bg-surface-container-lowest p-1.5 shadow-floating"
        >
...
function KindGlyph({ row }: { row: ConversationRowModel }) {
  ...
  if (row.kind === 'channel' && row.channel) {
    return (
      <span
        className="inline-flex items-center gap-1 flex-shrink-0 rounded-full bg-primary/15 px-1.5 py-0.5 text-xs font-medium text-primary"
        title={t('chat.fromChannel', { channel: row.channel.label })}
        aria-label={t('chat.fromChannel', { channel: row.channel.label })}
