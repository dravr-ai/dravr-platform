...
test.describe('Connections Tab - API Tokens Tab Visibility', () => {
  ...
  test('does not show Service Tokens tab for non-admin users', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ has: page.locator('span:has-text("Service Tokens")') })).not.toBeVisible();
  });
});
