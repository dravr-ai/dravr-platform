...
-- account deletion.
-- allowed_by is UUID to match users.id (cf. activity_fetch_freshness).
CREATE TABLE IF NOT EXISTS pre_approved_emails (
    email       TEXT PRIMARY KEY,      -- stored lowercase
    allowed_by  UUID,                  -- users.id of the approving operator; NULL pre-bootstrap
    note        TEXT,                  -- operator note (cohort, reason)
    created_at  TIMESTAMPTZ NOT NULL   -- when the allow was recorded
);
