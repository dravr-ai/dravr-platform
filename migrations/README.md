...
This directory is the **SQLite** tree. PostgreSQL has its own, `migrations_pg/`,
written natively for it — `UUID` primary keys with `gen_random_uuid()` defaults,
`TIMESTAMPTZ` timestamps with `now()` defaults, `JSONB`, `TEXT[]`, partial
indexes, and five `SERIAL` id columns on the usage-log tables. The conventions
below therefore describe the SQLite tree, not both; `schema_parity_test.rs`
holds the two to the same table set.

The SQLite tree uses **portable SQL syntax**:
...
1. **No database-generated defaults for timestamps** - Application provides all `created_at`/`updated_at` values (the PostgreSQL tree uses `TIMESTAMPTZ DEFAULT now()`)
2. **No AUTOINCREMENT** - All IDs are TEXT UUIDs generated by the application (the PostgreSQL tree differs: five usage-log tables use `SERIAL`)
