...
--   time_in_bed = end_time - start_time, in seconds
...
SET sleep_score = NULL,
    sleep_efficiency = CASE
        WHEN awake_seconds IS NOT NULL
         AND CAST(strftime('%s', end_time) AS INTEGER) - CAST(strftime('%s', start_time) AS INTEGER) > 0
         AND awake_seconds <= CAST(strftime('%s', end_time) AS INTEGER) - CAST(strftime('%s', start_time) AS INTEGER)
        THEN (CAST(strftime('%s', end_time) AS INTEGER) - CAST(strftime('%s', start_time) AS INTEGER) - awake_seconds) * 100.0
...
SET data_json = json_remove(data_json, '$.training_stress_score')
WHERE provider = 'whoop'
  AND json_extract(data_json, '$.training_stress_score') IS NOT NULL;
