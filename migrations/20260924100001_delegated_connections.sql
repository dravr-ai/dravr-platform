-- ABOUTME: A member's provider read through their group coach's session, proposed by the coach and confirmed by the member
...
CREATE TABLE IF NOT EXISTS delegated_connections (
    ...
    coach_tenant_id TEXT NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    member_user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    member_tenant_id TEXT REFERENCES tenants(id) ON DELETE CASCADE,
