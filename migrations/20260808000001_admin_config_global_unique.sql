...
-- SQLite treats NULL as distinct from every other NULL, so a tenant-less
-- override never collided with its predecessor and each save inserted another
-- row. Reads then took an arbitrary row, which in practice was the oldest, so
-- the effective value stayed pinned to the first-ever write.
--
